...
Similar to [Day 1 Default IngressController Subnet Selection on AWS](#day-1-default-ingresscontroller-subnet-selection-on-aws).
...
- Support updating the default subnets for all new IngressControllers after installation on AWS.
...
2. Add the `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets` and
   `spec.loadBalancer.platform.aws.networkLoadBalancer.subnets` fields in the IngressConfig
...
Ingress Operator will be updated to use the `Subnets` fields in the IngressConfig as the default subnets
...
fields will be added to the IngressConfig to encode the default subnets for all IngressControllers. Regardless of the
...
ID, the IngressConfig CRD will still support specifying subnets by name for completeness.
...
The Ingress Operator will be updated to consume the IngressConfig's `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets`
or `spec.loadBalancer.platform.aws.networkLoadBalancer.subnets` fields when creating IngressControllers. The
IngressConfig `subnet` field for the corresponding load balancer type will propagate to the IngressController's relevant
...
IngressConfig for the new load balancer type if no subnets are specified in the IngressController for that type. This
...
2. The OpenShift Installer installs the cluster into the VPC where the subnets exist, and populates the
   IngressConfig's `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets` and
...
default to using the subnets defined in the IngressConfig:
...
1. The cluster admin installs a cluster with IngressController-role subnets and a Classic load balancer for the
   `default` IngressController:
    ...
    [...]
    ```
2. The Installer populates these subnet values to the IngressConfig object:
      ```yaml
   apiVersion: config.openshift.io/v1
   kind: Ingress
   metadata:
   ...
   [...]
   ```
3. When the Ingress Operator starts, it uses the IngressConfig's `classicLoadBalancer` subnets value for the `default`
...
4. A cluster admin changes the `default` IngressController's load balancer type from `Classic` to `NLB`, but doesn't
   ...
   subnets specified in `spec.loadBalancer.aws.networkLoadBalancer` of the IngressConfig:
     ```yaml
   apiVersion: operator.openshift.io/v1
   kind: IngressController
   metadata:
...
- Q: Should the IngressConfig use a feature gate different from the `IngressControllerLBSubnetsAWS` feature gate
...
to the IngressConfig's `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets` and
