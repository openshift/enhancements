...
as well as the control plane load balancers and nodes. It also fixes a common issue where load balancers would
...
Adapted from [openshift/installer#6342](https://github.com/openshift/installer/pull/6342). See
[Specifying Manual Subnet Roles with BYO VPC during Installation Workflow](#specifying-manual-subnet-roles-with-byo-vpc-during-installation-workflow)
...
1. Deprecate the `platform.aws.subnets` field in the install-config and add a new
   `platform.aws.vpc.subnets` field, which allow users to explicitly manage subnet roles.
...
to support propagating explicit subnet selection to IngressControllers, the control plane load balancers, and nodes.
...
const (
    ...
    // IngressControllerLBSubnetRole specifies subnets used by the default IngressController and
...
`platform.aws.vpc.subnets` install-config field. The `AWSSubnets` struct is duplicated from the IngressController CRD
to maintain API consistency. Although the new `platform.aws.vpc.subnets` install-config field only supports specifying subnets by
...
Ingress Config `Subnets` field for the corresponding load balancer type will propagate to the IngressController's relevant
...
For example, they may want to override the default subnets for all IngressControllers (including the `default`
IngressController) during installation rather than relying on [AWS Subnet Discovery](#aws-subnet-discovery).
...
2. The OpenShift Installer uses the subnets based on their specified roles, configuring the ControlPlaneExternalLB,
   ...
   it populates the both the `default` IngressController and the `cluster` Ingress Config manifests with them and
   installs the cluster into the existing VPC:
   ```yaml
   ...
   spec:
     ...
     #...
   ```
3. The ControlPlaneLBs, ClusterNodes, and IngressControllers (`default` and user-created) are created using
   ...
   `default` IngressController:
    ...
    platform:
      aws:
        ...
        vpc:
          subnets:
          - id: subnet-0fcf8e0392f0910d0 # Private / AZ us-east-1a
            roles:
            - ControlPlaneInternalLB
            - ClusterNode
          - id: subnet-0fcf8e0392f0910d1 # Public / AZ us-east-1a
            roles:
            - IngressControllerLB
            - ControlPlaneExternalLB
        lbType: Classic
...
- The all-or-nothing approach in [Automatic vs. Manual Role Selection](#automatic-vs-manual-role-selection) prevents
  ...
  - For example, a cluster admin might want to isolate the ClusterNode subnet from the ControlPlaneInternalLB subnet,
    but leave the IngressControllerLB subnets unspecified to rely on subnet discovery for IngressControllers. This
