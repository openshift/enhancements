...
to skip security group creation when using powervc. The OpenStack team was not open to accepting this as they didn't want
to imply that all OpenStack clusters should consider not using security groups. We moved on from this approach to adding
a shim provider instead.

Leveraging the external provider was another option that we explored. PowerVC is unique in that it is a variant of
OpenStack and would not need as much customization as the external provider enables. Additionally, we felt that the
user experience would be easier if powervc was its own platform.
