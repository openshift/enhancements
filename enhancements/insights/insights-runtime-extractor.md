...
![insights-runtime-extractor workflow](./insights-runtime-extractor.1.png "Workflow of the Insights Runtime Extractor")
...
![insights-runtime-extractor implementation](./insights-runtime-extractor.2.png "Implementation of the insights-runtime-extractor")
...
The insights-runtime-extractor main program is a HTTP sever written in Rust.
The fingerprint programs are written in Rust (with some written in Go).

This image would contain:

* The HTTP server to access the extracted runtime data
  * Access to this HTTP server will be constrained to accept queries only from the `insights-operator` by using the https://github.com/openshift/kube-rbac-proxy[`kube-rbac-proxy`].
  * The Insights Operator will connect to the insights-runtime-extractor Pods through its DNS record 
* The `crictl` tool Find the running containers and their root PID
...
Due to the self-contained nature of these executables, the insights-runtime-extractor is built from `scratch` (and not from an existing base image).
