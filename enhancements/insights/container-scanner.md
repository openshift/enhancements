...
* Deploy the container scanner `DaemonSet` when gathering workload is starting
  * The DaemonSet has readiness probes to determine when it is ready to be used. The operator must wait until it is ready and if that takes too long, gather workload without scanning the containers. In that case, an error will be reported by the operator in the archive’s gathers.json file in the `"workloads/workload_info"` section.
...
* Determine the Pod IP address of each container-scanner’s Pod (as the container scanner is deployed as a DaemonSet, there is a single pod instance on each worker node) and map it to its worker node.
  * There is no service exposing the Container Scanner and its pods are accessed directly from their Pod IP address 
* For each running container, the Insights Operator determines which container scanner's Pod is running on the same worker node and sends an HTTP request with the ID of the container to scan. The HTTP response contains a JSON payload that is added to the data gathered by the operator.
* Undeploy the container scanner when gathering workload data is finished.
...
#### Container Scanner HTTP API

The Insights Operator communicates with the Container Scanner pods deployed by its DaemonSet with a simple HTTP API:

* `GET /scan?cid=cri-o://xx`
  * Scan the container with the corresponding container ID.
If the container ID is not found or there is no process corresponding to it, the HTTP server returns a `4xx` status code, otherwise, it returns a JSON payload with the extracted data:
...
* `GET /health/live`
  * Liveness & Startup probes for the container scanner
* `GET /health/ready`
  * Readiness probe for the container scanner
...
The container scanner main program is a HTTP server written in Rust.
The fingerprint programs are written in Rust (with some written in Go).

This image would contain a set of commands to:

* Start the HTTP server to query in order to scan a container (based on its container ID)
...
* Find the root PID based on a container ID with the `crictl` tool
...
In the existing initial implementation of the container scanner, it takes __~180 ms to scan a single container__ (including the HTTP roundtrip).

The current workflow for scanning the containers could be further optimized by performing the scanning after the container shapes have been tested for duplicates and scan only once instead of scanning every duplicated container.

It could also be possible to scan containers in parallel instead of doing it iteratively.
...
It requires integration tests with applications based on the detectable runtime stacks.
