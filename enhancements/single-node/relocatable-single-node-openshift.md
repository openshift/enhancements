...
Currently, when a single node OpenShift boots, ovs-configuration.service creates the br-ex bridge with the node’s default interface and sets the node’s IP as the environment variable for kubelet.service and crio.service.
...
During the initial installation at the factory, a MachineConfig will be applied that adds a NetworkManager configuration script that adds an IP to the br-ex bridge when created by ovs-configuration.service.
This IP will be the same IP as ovs-configuration.service configured on the br-ex bridge during the installation. 
The MachineConfig will also add an env override to kubelet.service and crio.service that sets the kubelet NODE_IP to the internal node IP added by the NetworkManager pre-up script.
...
#### SNO reconfiguration
We will add a new systemd service to reconfigure the node upon deployment at the edge site.
This new service will read the site-specific configuration (site-config) and reconfigure the following:

1. IP address and DNS server
The site config will contain the desired network configuration for the edge site.
...
Changing the pull secret is supported by OCP. In case the site-config contains pull secret, the re-configuration service will execute `oc set data secret/pull-secret -n openshift-config` with the new pull secret as described [here](https://access.redhat.com/solutions/4902871).
4. Reconfigure the ImageContentSourcePolicy
In case the site-config contains ImageContentSourcePolicy the re-configuration service will create a new ImageContentSourcePolicy resource with the updated policy.
...
Alternatively the user can configure a regular functional SNO (installed with supported installation method) to be relocatable after the SNO was successfully installed and already running.

Reconfiguration at the Edge site:
The site-config for the edge location can be delivered using a volume mount with 
to have a fully operational OpenShift cluster.
### API Extensions
...
Some day-2 operators (e.g. sriov-network-operator, performance-addon-operator) might not function properly if they are installed at the factory since they require the SNO to be fully configured with the edge site configuration before they are applied.
We can mitigate it by testing the deployment profile and in case the deay-2 operator requires the edge site confiugation it will get applied at the edge site instaed of at the factory.
...
In order to allow our sustomers to deploy edge clusters at scale we will need to create a Zero Touch Configuring service (similar to ZTP that only reconfigure the cluster instaed of provisionig it) to enable reconfiguring relocatable SNOs at scale.
...
Zero touch provisioning for factory workflows (ZTPFW), is an existing (unsupported) solution for  relocatable edge clusters, the ZTPFW requires additonal 
