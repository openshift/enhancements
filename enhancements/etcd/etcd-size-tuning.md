...
Specifically, in our build clusters, the number of namespaces and CRDs (and the size of the CRDs) have been increasing which drastically increase the amount of data stored in etcd.
In 4.14, the perf team is now unable to run their benchmark suite.
...
Allow the customer/admin access to set the backend quota directly through a human readable value via the etcd.operator.openshift.io/v1 techpreview field. The only restriction is that they are disallowed from setting the value lower than the current.
...
An initial upper limit of 64GiB will be enforced so as to limit the impact of some etcd operations, like restore, transporting snapshots, etc. 64GiB is an arbitrary value and work with the Perf-Scale team will need to be done to validate this value and potentially increase the upper limit; finding a reasonable and safe upper bound will need to be done before this feature is made GA.
...
- Add BackendQuotaGiB of type int64
  - With default '8GiB'.
  - With validation disallow a decrease in value.
  - With an upper limit of '64GiB'.

```go
type EtcdSpec struct {
  // BackendQuotaGiB sets the etcd backend storage size limit in gibibytes. Defaults to 8GiB.
...
This enhancement will not directly impact MicroShift as the api will be consume via the Cluster Etcd Operator.
