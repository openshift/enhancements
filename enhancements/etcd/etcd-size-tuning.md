...
* For a larger database, defragmentation of etcd will take longer which will increase the amount of time that etcd is unavaiable for writes. Etcd will still be available for reads during defragmentation, so there should be little impact on the apiserver's availability.
  * How long does it take to compact & defrag a database of a given size? Defragmentation times scale linearly with the DB size. Even at almost 16 GiB, however, we did not observe a concerning increase. Fixing 50% fragmentation usually takes 1-2 minutes.
  * There is a real risk that this will cause LE Lease updates to be blocked, so it can result in leader changes after the defrag completes.
We should measure how long it takes to compact & defrag a database of a given size (e.g. 8GiB vs 16GiB).
...
No special handling is required. The `.spec.backendQuotaGiB` field and its value are preserved
across upgrades and z-stream rollbacks. Y-stream downgrades are not supported by the CVO. If a
z-stream rollback occurs, the operator continues to respect the configured quota value.
