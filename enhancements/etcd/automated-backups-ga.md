...
- Retention rules allow managing the lifecycle of automated backups to prevent running out of disk space
   - Support additional retention rule types in future enhancement (e.g. MaxAge, MaxSize)
...
- Create a backup Job with `ttlSecondsAfterFinished` for GC and `operator.openshift.io/etcd-backup` finalizer
   ...
   - Use a reasonable TTL so that admins have an opportunity to inspect the job/pod/logs if needed
...
- Watch for Job success/failure
   - On success
       ...
       - Update `EtcdBackup` status with `BackupCompleted` condition, files, and nodeName
   - On failure
...
A new controller in the cluster-etcd-operator called BackupPolicyController reconciles backup schedule requests as follows:

- Watches `EtcdBackupPolicy` CRs
- Use robfig/cron parser to manage backup schedules internally
   - Using internal scheduling reduces the complexity of the kubernetes object model and gives cluster-etcd-operator full control over scheduling, retries, and failure
...
- Manage retention rules (outside of cron func)
   - List completed `EtcdBackups` and associate them with parent `EtcdBackupPolicy`
   - Sort by age and apply configured retention rules, deleting from oldest first
- On cron trigger, the BackupPolicyController creates a new `EtcdBackup` CR based on the `EtcdBackupPolicy`
   - Backup is labeled with `operator.openshift.io/etcd-backup-policy` label to identify the parent policy for retention rules
   - Backup does NOT recieve OwnerReference so they don't get deleted when a policy is removed
      - Backups will become orphaned in this case, and require manual cleanup. This is preferable over accidentally losing cluster backups.
...
- On GC Job success:
   - Remove the `operator.openshift.io/etcd-backup` finalizer from `EtcdBackups` and Job
- On GC Job failure:
   - Check if storage backend still exists. If not, backup can be finalized
   - Otherwise retry GC with exponential backoff
...
type EtcdBackupPolicySpec struct {
   ...
   // If a value greater than zero is set, nodes are chosen in a lexicographical order from the set of healthy nodes selected by the nodeSelector.
