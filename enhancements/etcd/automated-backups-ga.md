...
type EtcdBackupSpec struct {
   ...
   // If omitted, a random master node will be selected.
...
type EtcdBackupStatus struct {
   ...
   // If omitted, the backup file has not been created yet.
   // +kubebuilder:validation:Optional
   // +optional
...
type EtcdBackupPolicyRetentionRule struct {
   ...
   // +unionDiscriminator
   // +kubebuilder:validation:Enum:="MaxQuantity"
   // +kubebuilder:validation:Required
   // +required
...
Backups will be deleted in order from oldest to newest until all limits are satisfied. Backup deletion is managed by the PeriodicBackupController via the k8s API. Deleting an EtcdBackup signals the BackupGarbageCollectionController to delete the associated file from its storage backend.

In future, additional retention rules types can be added such as MaxAge and MaxSize.
