...
* Support smooth upgrades, preserving existing single-stack
...
4. When the user proceeds with a service recreation, the ingress-operator creates the
   LoadBalancer service with `ipFamilyPolicy: SingleStack` and
   `ipFamilies: ["IPv4"]`, falling back to IPv4-only configuration for
   that specific IngressController.
...
- On dual-stack clusters, when the load balancer type is CLB, the
  ingress-operator explicitly sets the service's `ipFamilyPolicy` to
  `SingleStack` and `ipFamilies` to `["IPv4"]`, since Classic Load
  Balancers do not support dual-stack networking. This is especially
  important on dual-stack clusters with IPv6 as the primary address,
  where `ipFamilies` will default to `["IPv6"]`, which instructs OVN
  to accept only IPv6 traffic, breaking IPv4 connectivity for CLB.
