...
* If there are no listeners with hostnames (and thus no listener-level `DNSReady` conditions), the gateway-level `DNSReady` condition will be removed from `status.conditions[]`.
...
and Gateway API `metav1.Condition` types:
  - `ComputeGatewayAPIDNSStatus`: Converts to listener-level conditions in `status.listeners[].conditions[]` and aggregates listener DNS conditions to a gateway-level condition in `status.conditions[]`
