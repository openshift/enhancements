...
- Validations done by API server as part of [ValidateHostUpdate()](https://github.com/openshift/openshift-apiserver/blob/bd2a35e58172010c658f4d8f4dff8f9f0eac187d/pkg/route/apiserver/registry/route/strategy.go#L96)
  ...
  Removal of tls certificate is allowed without these permissions check. This statement aligns with the existing implementation and also as mentioned [here](https://github.com/openshift/origin/pull/18177#issuecomment-360660024).
...
- Either `create` or `update` is required on route updatation with non-empty `.spec.tls.externalCertificate`. 
  ...
  - Few examples:
    - external-certificate-added-from-no-certificate
    - external-certificate-added-from-nil-tls
    - certificate-changed-to-external-certificate
    - external-certificate-secret-name-changed
    - **external-certificate-secret-name-unchanged**
- No permission is required to remove `.spec.tls.externalCertificate` from route.
  - Few examples:
    - external-certificate-removed-and-set-no-certificate
    - external-certificate-removed-and-set-nil-tls
