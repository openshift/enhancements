...
ProvisioningRequests to inform its scaling decisions so that it can create nodes atomically
in expectation of a user workload. This enhancement is about enabling the use of
...
be expected, and to create the nodes atomically for the workload. Under normal operation, the
...
- Adds the Kubernetes custom resource definition for `provisioningrequests.autoscaling.x-k8s.io/v1`
...
For OpenShift Kubernetes Engine installations that contain cluster autoscaling as managed by
the Cluster Autoscaler Operator, the changes described in this enhancement should work in
a similar manner as standalone OpenShift.
...
To mitigate the possibility of a resource definition clash, when performing an upgrade to
a version that enables the ProvisioningRequest an administrator will need to acknowledge
that OpenShift will control the lifecycle of the ProvisioningRequest resource definition.
...
_Answered during review:_

At this time we do not see a need to allow disabling of the ProvisioningRequest resource by
the user. If a cluster administrator needs to restrict access to this resource, they should
use resource based access controls to limit users.

2. Should ProvisioningRequests have a capability to allow targeted installation of this feature?

_Answer from discussions_

We currently do not have a capability for autoscaling, we would first want a general capability
to cover all cluster autoscaling first. It is unlikely that we would want a capability only
for ProvisioningRequests, but it "predictive autoscaling" might be a category for consideration
in the future.
...
Cluster Autoscaler after an upgrade to change the Deployment configurations. To support this
activity, an administrator will need to provide an acknowledgement when upgrading to
a version with ProvisioningRequests to ensure that they have not installed a custom version
of this resource.
...
- When upgrading from OpenShift versions less than 5.0 to a version 5.0 or greater,
  the Cluster Autoscaler will be restarted to add the command line flag that enables
  ProvisioningRequest support.
- When upgrading to an OpenShift version that supplies ProvisioningRequest in general
  availability, administrators will need to provide an acknowledgement during the upgrade
  to confirm that OpenShift will now be responsible for managing ProvisioningRequest
  resources, to prevent clash with a custom resource definition.

Downgrade expectations:
- When downgrading to OpenShift versions less than 5.N from a version equal to or
  greater than 5.N, the ProvisioningRequest resource definition will be removed.
- When downgrading to OpenShift versions less than 5.N from a version equal to or
  greater than 5.N, the Cluster Autoscaler will be restarted to remove the command
