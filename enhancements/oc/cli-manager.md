...
and this makes it difficult for disconnected installations to pull them.
...
binaries/tools/CLIs. Also, these artifacts will be accessible via `oc` and can be made downloadable from the console, if desired.
...
for `oc` will be extracted from. If other CLIs choose to offer artifacts in this way, they can opt in by supplying an image to be
pulled from their desired image registry.
...
Through images and ImageContentSourcePolicy, CLIs will be available to disconnected environments. Currently, the console provides
...
* A single ClusterCLI CR should accept multiple versions, with each version supporting multiple platforms
* Must work with image registries that require image pull secrets
* An index with artifacts similar to [index of /pub/openshift-v4/clients](https://mirror.openshift.com/pub/openshift-v4/clients/) with added listing of CLI images
