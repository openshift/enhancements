...
allows the administrator to disable the network diagnostics. With this enhancement CNO would go 
degraded if `networkDiagnostics` is configured when `disableNetworkDiagnostics` is set. 
...
const (
	...
	// NetworkDiagnosticsDisabled means that network diagnostics checks are disabled
...
type NetworkDiagnosticsNodePlacement struct {
	// nodeSelector is the node selector applied to a network diagnostics component
	//
	...
	// tolerations is a list of tolerations applied to a network diagnostics component
	//
