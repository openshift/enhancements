...
OVN-Kubernetes operators, support engineers, and end users investigating connectivity and routing on OpenShift often need Northbound and Southbound database views (`ovn-nbctl`, `ovn-sbctl`, traces, logical flows), OVS bridge and OpenFlow inspection (`ovs-ofctl` and related helpers), host-oriented diagnostics, and packet or kernel-level capture workflows. **openshift-mcp-server today does not expose these core networking troubleshooting tools**, even though ovn-kubernetes-mcp already implements them upstream.

The primary motivation for this enhancement is to **close that gap** by making the existing ovn-kubernetes-mcp toolset available in OpenShift so support, operators, and end users can diagnose OVN-Kubernetes networking issues through the product MCP surface, reusing upstream implementations rather than re-implementing or forking equivalent tool logic.

Delivering those tools through openshift-mcp-server also gives customers **a single MCP server** for OpenShift troubleshooting: the same server and configuration used for broader Kubernetes and platform operations can expose OVN-Kubernetes diagnostics, instead of deploying and maintaining a separate MCP server with its own endpoints, credentials, access policies, and day-two operations. By integrating into the existing openshift-mcp-server, OpenShift avoids funding a parallel pipeline for a standalone OVN MCP offering with separate build and release pipelines, container image lifecycle and CVE response, supportability and documentation matrices, security review surfaces, and client onboarding for another server identity. OVN-Kubernetes tooling then inherits the same packaging, upgrade, and support expectations of openshift-mcp-server, which is materially cheaper to own than treating OVN-Kubernetes diagnostics as a second shipped product.
