...
To minimize CI costs while ensuring comprehensive coverage, no-overlay tests are organized as follows:
- **Unmanaged mode (CUDNs):** Tested in existing BGP CI lanes by adding no-overlay CUDN tests
- **Managed mode (default network + CUDNs):** Tested in new dedicated CI lanes

#### Testing unmanaged mode
...
#### Testing managed mode

We can test the default network in two new separate CI lanes that we create specifically for the managed scenario, where we can test no-overlay mode with the full mesh topology. We can have one lane run ovn-kubernetes in shared gateway mode and the other in local gateway mode. The default network will operate in no-overlay mode. To cover different `outboundSNAT` configurations for the default network, one lane can test `outboundSNAT`=`Enabled` and the other `outboundSNAT`=`Disabled` (configured after installation). We will then dynamically create Cluster User-Defined Networks (CUDNs) in no-overlay mode, first with `outboundSNAT` set to `Enabled` and then to `Disabled`. This approach allows us to cover both SNAT configurations within the same lane. The proposed lane names are:
