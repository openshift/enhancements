...
### User Stories
TODO: Replace commit hash with "master" once the PR merges:
...
Currently, BGP is only supported on Bare Metal clusters. Since no-overlay mode requires BGP, it shares this limitation for now. As BGP support expands to more platforms, so will no-overlay mode.
...
- Cluster User Defined Networks (CUDNs), at run time
...
The cluster administrator should also provide in the `manifests` folder the necessary manifests for the `unmanaged` scenario, where the external BGP routers are defined:
  - FRRConfig CR
  - RouteAdvertisements CR

Any error in setting up no-overlay mode for the default network will result in ovn-kubernetes in crashLoopBackOffError.
<!-- TODO: Expand on the FRRConfig and RA   -->
...
In `operator.openshift.io/v1 Network` the cluster administrator should enable no-overlay mode for the default network and configure it with the necessary per-network parameters:
  - `outboundSNAT`: `enable`,`disable`
  - `routing`: `managed`
In addition to that, the managed routing options should also be set in `operator.openshift.io/v1 Network`: `asNumber` and `bgpTopology` for the configuration to be valid.
