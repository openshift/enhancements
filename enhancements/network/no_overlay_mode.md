...
- enable no-overlay mode for the default network and configure it with the per-network parameters:
  - outboundSNAT: "enable","disable"
  - "routing": "managed", "unmanaged"
- configure the "managed" scenario, when routing=managed:
  - asNumber
  - bgpTopology: "routeReflector", "fullMesh".

CNO will then propagate these configuration parameters to the ovn-kubernetes components (ovnkube-control-plane and ovnkube-node) via the existing configmap. Changes to the no-overlay mode configuration after the initial cluster installation are not currently supported and are therefore forbidden by the newly introduced API.
...
Following BGP requirements, a CUDN is eligible for no-overlay mode if its subnet doesn't overlap with the subnet of other CUDNs that are in no-overlay mode. A status message on the CUDN will report failure to enable no-overlay mode in case that happens.


<!-- Furthermore, since no-overlay mode builds on top of BGP, it needs: -->
<!-- - a RouteAdvertisement (RA) CR to export default-network and/or CUDN routes  -->
<!-- - a FRRConfig CR to import such routes to cluster nodes from neighbor routers -->
...
index 111240eec..1c37ffe4d 100644
--- a/operator/v1/types_network.go
+++ b/operator/v1/types_network.go
@@ -399,6 +399,8 @@ type OpenShiftSDNConfig struct {
...
@@ -468,6 +470,45 @@ type OVNKubernetesConfig struct {
...
+type NoOverlayManagedConfig struct {
...
@@ -898,3 +939,36 @@ type AdditionalRoutingCapabilities struct {
...
+
+       BGPTopologyRouteReflector BGPTopology = "routeReflector"
...
In our scale testing we have good coverage for clusters of size: small (24 nodes), medium (120 nodes), large (250 nodes); x-large (500 nodes) clusters are less frequently tested. We should aim to have coverage for no-overlay mode at least up to medium-size clusters (120 nodes) and verify that a full mesh topology is sustainable at this scale.
