...
When creating the synthetic network-selection-element for plumbing the primary
UDN attachment, the OVN-Kubernetes control plane will decorate it with the
gateway which was configured by the user in the UDN/C-UDN.

The primary UDN gateway code will also need to updated to parametrize the
gateway IP address, since today it is using the UDN subnet first IP address.
