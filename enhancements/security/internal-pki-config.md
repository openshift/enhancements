...
This proposal adds two new resources in the `config.openshift.io` API group:

- **PKI**: A cluster-level configuration resource that allows administrators to specify cryptographic parameters for different categories of certificates (signer, serving, client) with support for fine-grained overrides of specific named certificates.
- **PKICertificateDefinition**: A namespaced resource that allows operators to dynamically register the certificates they manage, enabling validation of certificate names in PKI overrides without hardcoding them in the CRD.
...
This proposal introduces two new resources in the `config.openshift.io/v1alpha1` API group, along with a `ConfigurablePKI` feature gate to control the rollout:

- **PKI**: A cluster-scoped singleton configuration resource that allows administrators to specify cryptographic parameters for internal certificates organized by category and name.
- **PKICertificateDefinition**: A namespaced resource that allows operators to register the certificates they manage, enabling dynamic validation of certificate names in PKI overrides.
...
1. **New API Resources**:
   ...
   - `PKICertificateDefinition` resource in `config.openshift.io/v1alpha1` (namespaced, for dynamic certificate registration)
...
3. **ValidatingAdmissionPolicy**: Validates that certificate names in PKI overrides reference certificates registered in PKICertificateDefinition resources
4. **Installer Integration**: Limited Day-1 configuration support for signer certificate cryptographic parameters
5. **Operator Updates**: Modifications to certificate-generating operators to:
   - Create `PKICertificateDefinition` resources to register their managed certificates
   - Watch and consume the PKI configuration independently
6. **Certificate Rotation**: Operators apply PKI configuration parameters during existing certificate rotation cycles (no changes to rotation mechanisms themselves)
7. **Metrics and Observability**: Expose metrics for certificate generation events and configuration compliance

Note: There is **no central PKI controller**. Each certificate-generating operator watches the PKI resource directly, manages its own `PKICertificateDefinition`, and applies configuration to its own certificates.
...
This enhancement adds two new Custom Resource Definitions (CRDs) to the OpenShift API:

- **PKI**: Cluster-scoped singleton for configuring certificate cryptographic parameters
- **PKICertificateDefinition**: Namespaced resource for operators to register their managed certificates

#### Compatibility Level

Both the PKI and PKICertificateDefinition APIs will be developed initially at **Compatibility Level 4** (TechPreviewNoUpgrade) and graduate to **Compatibility Level 1** (GA) before the target OpenShift release.
...
type PKICertificateManagement struct {
    ...
    //
    // Unmanaged: Components use their existing hardcoded certificate generation behavior.
    // Default: Use OpenShift-recommended best practices for certificate generation.
    // Custom: Administrator explicitly configures cryptographic parameters via the custom field.
...
// PKIProfile defines the certificate generation parameters. Configuration can be specified
// at three hierarchical levels: defaults, categories, and overrides.
// More specific levels take precedence over general ones.
type PKIProfile struct {
    // defaults specifies the default certificate configuration for all certificates
    // unless overridden by category or specific certificate configuration.
    Defaults CertificateConfig `json:"defaults,omitzero"`

    // categories allows configuration of certificate parameters for categories of certificates
    // (SignerCertificate, ServingCertificate, ClientCertificate).
    Categories []CategoryCertificateConfig `json:"categories,omitempty"`

    // overrides allows configuration of certificate parameters for specific named certificates.
...
// This is a discriminated union: algorithm is the discriminator,
// and exactly one of rsa or ecdsa must be set based on the algorithm.
type KeyConfig struct {
    ...
    // rsa specifies RSA key parameters. Required when algorithm is RSA.
    RSA RSAKeyConfig `json:"rsa,omitzero"`

    // ecdsa specifies ECDSA key parameters. Required when algorithm is ECDSA.
...
#### PKICertificateDefinition Resource

The `PKICertificateDefinition` resource is a namespaced resource (in `openshift-config`) that allows components to register their managed certificates for dynamic validation of certificate names in PKI overrides.
...
type PKICertificateDefinitionSpec struct {
    ...
    // Example: "etcd-operator", "kube-apiserver-operator"
...
type CertificateDefinition struct {
    ...
    // Used in PKI.spec.certificateManagement.custom.overrides[].certificateName.
    Name string `json:"name,omitempty"`

    // category specifies the certificate category (SignerCertificate, ServingCertificate, ClientCertificate).
    Category CertificateCategory `json:"category,omitempty"`
...
#### Dynamic Certificate Registration

Certificate names used in `overrides` are dynamically registered by operators via `PKICertificateDefinition` resources. Each operator creates a `PKICertificateDefinition` in the `openshift-config` namespace to declare the certificates it manages.

**Example PKICertificateDefinition:**
...
A `ValidatingAdmissionPolicy` validates that certificate names in `PKI.spec.certificateManagement.custom.overrides` reference certificates registered in `PKICertificateDefinition` resources. This enables:

- **Dynamic extensibility**: Operators can register new certificates without CRD updates
- **Self-documenting**: Users can list available certificates with `oc get pkicertificatedefinitions -n openshift-config`
- **Conflict detection**: CEL validation rules prevent duplicate certificate names
- **No central controller needed**: Each operator manages its own `PKICertificateDefinition` independently

#### Well-Known Certificate Names

The following certificate names are registered by OpenShift operators and can be used in `overrides` for fine-grained configuration.
...
**Note:** This list reflects certificates registered via `PKICertificateDefinition` resources. Additional certificates may be registered by other operators as they adopt the PKI configuration API. Use `oc get pkicertificatedefinitions -n openshift-config -o yaml` to see all registered certificates.
...
1. Create a `PKICertificateDefinition` resource in `openshift-config` to register its managed certificates
2. Watch the `PKI` cluster resource for changes
3. Implement configuration resolution logic to determine parameters for each certificate
4. Apply new parameters during the next certificate rotation (natural expiry or forced)
5. Report metrics on certificate generation events
...
The PKI API uses a combination of **CRD-level CEL validation** and **ValidatingAdmissionPolicy** for comprehensive validation.
...
3. **Enum and range constraints:**
   - `mode`: Unmanaged, Default, Custom
   - `algorithm`: RSA, ECDSA
   - `keySize`: Multiples of 1024 from 2048 to 8192
   - `curve`: P256, P384, P521
   - `category`: SignerCertificate, ServingCertificate, ClientCertificate

4. **Certificate name format** (`CertificateOverride` type):
   - Must match DNS subdomain pattern: `^[a-z0-9]([-a-z0-9]*[a-z0-9])?$`

**CRD-Level CEL Validation (PKICertificateDefinition resource):**

1. **Namespace constraint**: Must be created in `openshift-config` namespace
2. **Certificate name format**: Each certificate name must match DNS subdomain pattern

**ValidatingAdmissionPolicy (Dynamic Certificate Name Validation):**

A `ValidatingAdmissionPolicy` validates that certificate names in `PKI.spec.certificateManagement.custom.overrides` reference certificates registered in `PKICertificateDefinition` resources:
...
spec:
  ...
  - expression: |
    ...
    message: "certificateName must reference a certificate registered in a PKICertificateDefinition"
```
...
- PKICertificateDefinition API validation (namespace constraint, name format)
...
- Verify operators create PKICertificateDefinition resources
...
- Test ValidatingAdmissionPolicy rejects invalid certificate names in overrides
...
- Verify PKICertificateDefinition resources are created by operators
- Verify override validation rejects unregistered certificate names
...
- PKI and PKICertificateDefinition CRDs installed
- ValidatingAdmissionPolicy for certificate name validation deployed
...
- At least kube-apiserver-operator, etcd-operator, and service-ca-operator:
  - Create PKICertificateDefinition resources to register their certificates
...
- **APIs promoted to v1 at Compatibility Level 1:**
  - Both PKI and PKICertificateDefinition APIs stable
  - Breaking changes no longer allowed
  - API stable within major release for 12 months or 3 minor releases
...
1. The PKI and PKICertificateDefinition CRDs are installed during upgrade
2. A PKI resource is automatically created with `spec.certificateManagement.mode: Unmanaged`
3. Operators create their `PKICertificateDefinition` resources in `openshift-config` to register their certificates
4. With `spec.certificateManagement.mode: Unmanaged`, operators use their existing hardcoded defaults, ensuring zero behavior change
...
- Operators create their `PKICertificateDefinition` resources as they upgrade
...
### PKI and PKICertificateDefinition CRDs

**SLIs:**
- PKI resource exists and is readable: `GET /apis/config.openshift.io/v1alpha1/pkis/cluster` returns 200
- PKICertificateDefinition resources exist: Operators create their certificate registration resources in `openshift-config`
- CEL validation functions correctly: PKI and PKICertificateDefinition resource creation/updates succeed with valid configuration and are rejected with invalid configuration
- ValidatingAdmissionPolicy works correctly: PKI overrides referencing unregistered certificates are rejected
...
4. **Unregistered Certificate Name in Override**:
   - *Symptom*: PKI resource update is rejected by ValidatingAdmissionPolicy
   - *Impact*: Configuration change is blocked
   - *Mitigation*: Check available certificate names with `oc get pkicertificatedefinitions -n openshift-config -o yaml`
   - *Detection*: Client receives error message indicating the certificate name is not registered

5. **Missing PKICertificateDefinition**:
   - *Symptom*: Operator hasn't created its PKICertificateDefinition resource
   - *Impact*: Certificate names from that operator cannot be used in overrides
   - *Mitigation*: Check operator logs, verify operator is running and has correct RBAC
   - *Detection*: `oc get pkicertificatedefinitions -n openshift-config` shows missing operator
...
2. Check PKICertificateDefinition resources exist:
   ```bash
   oc get pkicertificatedefinitions -n openshift-config
   ```

3. Verify ConfigurablePKI feature gate is enabled:
   ```bash
   oc get featuregate cluster -o yaml | grep ConfigurablePKI
   ```

4. Check certificate-generating operator status:
   ```bash
   oc get clusteroperator kube-apiserver -o yaml
   # Look for Degraded=True conditions related to PKI
   ```

5. Review operator logs for PKI-related errors:
   ```bash
   oc logs -n openshift-kube-apiserver-operator deployment/kube-apiserver-operator | grep -i pki
   ```

6. Check metrics for certificate generation events:
   ```promql
...
3. Compare against PKI configuration:
   ```bash
   oc get pki cluster -o jsonpath='{.spec.certificateManagement.custom.categories[?(@.category=="ServingCertificate")].certificate.key}'
   ```
