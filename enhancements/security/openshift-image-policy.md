...
The ClusterImagePolicy CRD and controller are gated by the `ImagePolicy` feature gate, which is [currently `TechPreviewNoUpgrade`][feature-gate].
The new `openshift` ClusterImagePolicy would be initially gated by that same `ImagePolicy` feature gate, and so also initially `TechPreviewNoUpgrade`.
...
Instead of [the *Proposal*](#proposal)'s environment variable, the installer could add an install-config property like `internalTestingImagePolicy: Disable` (also gated by the `ImagePolicy` feature gate, [precedent for a feature-gated install-config property][install-config-feature-gate-precedent]).
