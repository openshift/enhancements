...
reviewers: # Include a comment about what domain expertise a reviewer is expected to bring and what area of the enhancement you expect them to focus on. For example: - "@networkguru, for networking aspects, please look at IP bootstrapping aspect"
  - "@ShazaAldawamneh" # @ShazaAldawamneh for rearchitecture details
...
- As a cluster administrator, I want changes to the External OIDC configuration to avoid rolling
  out kube-apiserver pods, so that authentication remains available while the configuration is being updated.
...
- **Extensibility.** Provide an extensible authentication integration point so that new authentication
  functionality can be implemented without modifying the kube-apiserver or carrying downstream patches.
...
- **Eliminate all kube-apiserver rollouts.** The goal is to decouple External OIDC configuration changes
  from KAS rollouts, not to eliminate rollouts caused by unrelated control-plane changes.
...
- When opting into External OIDC, instead of configuring the KAS with a `JWTAuthenticator`
  ...
  consumes the External OIDC structured authentication configuration. The KAS retains a stable
  webhook authenticator configuration regardless of whether the webhook serves Integrated OAuth
  or External OIDC.
...
```mermaid
flowchart TD
    A[Cluster administrator] -->|Configures| B[Authentication resource]
...
2. The kube-apiserver (KAS) invokes configured authenticators in sequence and proceeds to the webhook if earlier
   ...
   the chain
3. The kube-apiserver invokes the webhook by making a `TokenReview` request to the dedicated webhook Service
...
`authentication.config.openshift.io/v1` `Authentication` resource named `cluster`.

#### Hypershift / Hosted Control Planes

In HyperShift, the HCPO manages the authentication stack and the `oauth-apiserver`
deployment and configuration within the hosted control plane. The HCPO also manages
the hosted KAS-side webhook configuration and configures the hosted KAS to communicate
with the `oauth-apiserver`. Cluster administrators configure External OIDC through
`HostedCluster.spec.configuration.authentication`; the HCPO propagates this configuration
into the hosted control plane. The `oauth-apiserver` and its dedicated webhook Service
run within the hosted control plane, and the hosted KAS reaches the webhook through that
Service.
...
The existing `openshift-oauth-apiserver/api` Service used to configure the Integrated OAuth webhook remains
unchanged and is reused by the External OIDC webhook. As a result, the KAS-facing webhook configuration is
the same for `IntegratedOAuth` and `OIDC`; only the internal authentication mode of the webhook changes.
...
- Update the HCPO kube-apiserver configuration generation to keep the webhook authenticator configuration,
  point it to the dedicated external OIDC component's Service, and drop configuration via the structured
  authentication file (i.e. flag `--authentication-config`).
...
- Ensure that secrets required by the new component (credentials, CA data) are available on the management
...
- Yet another middleman component for authentication (we were trying to move away from the OAuth server,
  this moves us back to it)
...
- Reuses the existing `oauth-apiserver` deployment, certificate, health-check, and operator-management patterns
- Creates a stable architectural foundation for independently delivered features such as external claims sourcing

## Alternatives (Not Implemented)

- **Keep direct KAS structured authentication:** preserves the current architecture but leaves External OIDC coupled to KAS configuration and rollout behavior.
...
- The architecture is controlled by its own feature gate and is independent of `ExternalOIDCExternalClaimsSourcing`.
...
- Upgrade, downgrade, configuration update, invalid configuration, and webhook rollout scenarios are covered.
