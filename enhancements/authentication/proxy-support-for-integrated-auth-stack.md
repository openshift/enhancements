...
- Proxy support for the OAuth API Server's External OIDC mode. External OIDC is gated behind DevPreviewNoUpgrade and its egress is limited to the `oauth-apiserver external-oidc` subcommand. Proxy support for that code path can be added when External OIDC matures.
