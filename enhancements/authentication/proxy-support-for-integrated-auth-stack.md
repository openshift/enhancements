...
3. **In-cluster forward proxy** — deploy a proxy server in a pod, point the cluster-wide proxy at it, and configure it to
   selectively route auth-related traffic to the external IdP while dropping or redirecting everything else. This works but is
   entirely custom — the proxy configuration, routing rules, and TLS handling are the administrator's responsibility with no
   platform support or upgrade guarantees.

4. **Internal IdP federation** — deploy an internal identity provider (e.g., Keycloak) that federates to the external IdP, and only allow
   ...
   and upgrade, with its own HA, certificates, and storage requirements. It also increases auth latency.

5. **Egress sidecar** — inject a sidecar proxy (e.g., Envoy) into the OAuth Server pod to intercept outbound IdP traffic and forward it
...
6. **Manual env var injection** — skip the cluster-wide proxy API entirely and patch the OAuth Server and operator deployments directly with proxy env vars.
...
2. `spec.proxy` is picked up by the operator, causing it to:
   ...
   3. re-deploy `oauth-server` with new proxy environment variable values.
      When `trustedCA` is set, the referenced ConfigMap is synchronized into
      `openshift-authentication` and mounted into the OAuth Server pod. The
      OAuth Server watches the mounted CA file and hot-reloads it on change,
      so subsequent `trustedCA` content updates do not require a redeployment.
...
with one difference: `noProxy` uses `[]string` (a set of individual entries) instead of a
comma-separated string. This avoids parsing ambiguity and enables per-item CRD validation.
...
type AuthenticationProxyConfig struct {
	...
	// the proxy should not be used. Each entry must be between 1 and
	// 253 characters long and at most 64 entries are allowed. Duplicate
...
URL format is validated at the CRD level: `isURL(self)` rejects unparseable URLs, additional
CEL rules enforce `http`/`https` scheme, a non-empty hostname, and reject path, query parameters,
and fragments — only scheme, userinfo (for proxy authentication), and host:port are meaningful
to Go's HTTP transport. `MaxLength=2048` bounds CEL cost estimation.

`noProxy` is a set of strings with `MaxItems=64`. No further operator-level validation is
performed — invalid entries are silently ignored by the HTTP transport.

The `trustedCA` content and proxy connectivity are validated at runtime by the proxy validation controller, which sets
`Degraded` on invalid configuration.
...
When the component-scoped proxy is active, the operator appends cluster-internal defaults
(`.cluster.local`, `.svc`, `127.0.0.1`, `localhost`) to the user-provided `noProxy` value.
When `noProxy` is omitted, only the defaults are used.
...
|------|------------|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Integrated OAuth | Config observation | [`ObserveIdentityProviders()`](https://github.com/openshift/cluster-authentication-operator/blob/9d5f19f8ad67/pkg/controllers/configobservation/oauth/observe_idps.go#L19) | 1. [`discoverOpenIDURLs()`](https://github.com/openshift/cluster-authentication-operator/blob/9d5f19f8ad67/pkg/controllers/configobservation/oauth/idp_conversions.go#L315) <br> 2. [`checkOIDCPasswordGrantFlow()`](https://github.com/openshift/cluster-authentication-operator/blob/9d5f19f8ad67/pkg/controllers/configobservation/oauth/idp_conversions.go#L373) |
...
OAuth Server pod spec. Changes to proxy environment variables trigger a redeployment.
The OAuth Server's HTTP transports already respect proxy environment variables, so no
application-level code changes are needed for proxy URL handling.

When `spec.proxy.trustedCA` is set, the referenced ConfigMap is synced from `openshift-config`
to `openshift-authentication` and mounted as a volume in the OAuth Server pod. The OAuth
Server watches the mounted CA file and hot-reloads it on change, rebuilding its outbound HTTP
transports with the updated certificate pool. This avoids a redeployment on CA rotation.

CAO must watch the source ConfigMap in `openshift-config` and copy it into
`openshift-authentication` on any change. A redeployment is only triggered when the
volume/mount configuration changes (adding or removing `trustedCA`), not on CA content updates.
...
On single-node deployments, a proxy URL change triggers an OAuth Server redeployment,
causing a brief authentication outage (same as any OAuth config change on SNO today).
This is inherent to the single-replica topology and should be documented. CA bundle
updates (`trustedCA` content changes) are hot-reloaded without redeployment, avoiding
disruption for certificate rotation.
...
Testing is organized into three areas:

**1. Input validation** (CRD integration tests, API repo)

CEL validation rules on proxy URL fields (`httpProxy`, `httpsProxy`) and the
`trustedCA.name` reference are covered by declarative CRD tests in
`operator/v1/tests/authentications.operator.openshift.io/`. These verify that
invalid input (bad scheme, missing hostname, path/query/fragment, invalid
ConfigMap name) is rejected at admission time.

**2. Authentication flow** (E2E tests, CAO repo)

An external IdP (Keycloak) is deployed in the test cluster behind a forward proxy.
The IdP is reachable only through the proxy.

*Operator (config observation):*

- Adding/updating an OIDC IdP with `spec.proxy` set — the operator validates
  the IdP configuration (OIDC discovery) through the component proxy without
  errors.
- Removing `spec.proxy` — the operator falls back to the cluster-wide proxy
  (or direct connectivity) for IdP validation.

*Operand (OAuth Server):*

- OIDC login succeeds through the configured component proxy.
- Removing `spec.proxy` — login falls back to the cluster-wide proxy (or direct
  connectivity).
- Updating the `trustedCA` ConfigMap content is picked up by the OAuth Server
  without redeployment (CA hot-reload via file watcher).

**3. Operator health** (E2E tests, CAO repo)

- Misconfigured proxy URL (unreachable host, wrong port) sets `Degraded`
  on the proxy validation controller.
- Unreachable IdP endpoint emits `IdPEndpointUnreachable` Warning event
  (does not set `Degraded`).
- Endpoint accessibility and custom route health checks succeed through the
  component proxy. These are verified implicitly: both controllers must
  succeed for the operator to reach `Available`, and the authentication
  flow tests wait for `Available` before running. A proxy wiring failure
  in either controller would surface as `Degraded`/`Progressing` and
  block the E2E test setup.

**4. Proxy resolution** (unit tests, CAO repo)

The logic that selects component-scoped proxy vs cluster-wide fallback
vs no proxy is covered by unit tests.

Upgrade is tested by the standard CI upgrade jobs — the feature is gated
behind `AuthenticationComponentProxy` and the field is ignored by older operators.

## Graduation Criteria

### Dev Preview -> Tech Preview

N/A — feature ships directly as Tech Preview behind the
`AuthenticationComponentProxy` feature gate.

### Tech Preview -> GA

- Feature works end-to-end behind `AuthenticationComponentProxy` feature gate
- Unit test coverage for proxy resolution logic
- CRD integration tests for CEL validation rules in the API repo
- E2E tests running in CI (presubmit and periodic)
- Tests meet [promotion requirements](https://github.com/openshift/enhancements/blob/master/dev-guide/feature-zero-to-hero.md#promotion-requirements):
  run across supported platforms, pass rate >= 95%, in place >= 14 days before branch cut
- Upgrade testing with `spec.proxy` configured
- User-facing documentation in [openshift-docs](https://github.com/openshift/openshift-docs/)
- Sufficient soak time for feedback from Tech Preview users
