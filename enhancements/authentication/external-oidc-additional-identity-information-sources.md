...
The `externaloidc/apis/authentication/v1alpha1` type is an internal
configuration format for `oauth-apiserver`, not a served API or CRD. The Cluster
Authentication Operator in Standalone Openshift and the Control Plane Operator in
HyperShift generate it from the existing OpenShift authentication configuration,
and only `oauth-apiserver` consumes it; the kube-apiserver consumes the webhook's
`TokenReview` endpoint. The external-claims functionality and its user-facing
fields remain disabled by default behind the `ExternalOIDCExternalClaimsSourcing` feature gate.
...
As part of the CAO implementation of this feature, the CAO uses an [oauth-apiserver generator](https://github.com/openshift/cluster-authentication-operator/blob/28632f237aa003bb191100eabdc46c3e20c1fb49/pkg/controllers/externaloidc/generation/oauthapiserver/generate.go) that produces the oauth-apiserver's `AuthenticationConfiguration` type (`externaloidc/apis/authentication/v1alpha1`, including the `externalClaimsSources` fields introduced with this EP). Since both topologies need to generate the same configuration for the new dedicated component, this generator should be extracted to a shared library and reused in HyperShift.
...
The HyperShift Operator (HO) runs on the management cluster and manages `HostedCluster` resources across multiple OCP minor releases simultaneously. It validates authentication configuration during reconciliation. The existing OIDC CEL validation is configured with the minimum supported OCP version, but release-specific CEL compatibility remains unresolved.
