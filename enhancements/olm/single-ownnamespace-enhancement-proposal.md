...
4. **Install Mode Detection**: The system automatically determines the install mode according to the citerion table enumerated in the `Implementation Details/Notes/Constraints` section. 

A scenior exists where the user must specify the watch namespace. Example workflow of a bundle that will require the watch namespace to be specified: 
...
2. ClusterExtension does not install. The `Installing` and `Progressing` conditions will be set to false with an error that indicates
the required `watchNamespace` is not specified
...
type ClusterExtensionConfig struct {
	...
	//
	// The Type=object validation ensures that inputs are valid key/value
	// configuration objects, rejecting valid JSON that isn't configuration-like
	// (e.g., boolean `true`/`false`, arrays, or strings) and enforcing object structure
	// resembling configuration files with key: value pairs.
	//
