...
Since this approach adds a gRPC endpoint to the existing CatalogSource catalog pods (no new pods), it inherits the existing CatalogSource topology in HyperShift environments. The Console backend needs network access to the CatalogSource catalog pods' gRPC service. The specifics of how this works in HCP are still being investigated and this enhancement will be updated once we have a clear path.
...
- The lifecycle UI is gated behind the `OPERATOR_LIFECYCLE_METADATA` feature flag, which is only enabled when the TechPreviewNoUpgrade feature set is active. On clusters without Tech Preview enabled, the lifecycle columns are not rendered.
- The `ExperimentalListPackageCustomSchemas` endpoint remains available on catalog pods regardless, but is not consumed when the Console feature flag is absent.
...
**Recovery:**
- Enabling TechPreviewNoUpgrade on a new cluster activates the lifecycle columns in the Console
