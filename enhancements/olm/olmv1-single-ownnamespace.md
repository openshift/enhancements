---
title: olmv1-single-ownnamespace
authors:
  - "@Nico Schieder"
...
tracking-link: # link to the tracking ticket (for example: Jira Feature or Epic ticket) that corresponds to this enhancement
  - https://issues.redhat.com/browse/OPRUN-3767
# see-also:
...
# Single/OwnNamespace support for OLMv1
...
- **multi-tenant / multi-version operators**\
Implementation of this brief does NOT enable support for installing bundles multiple times on the same cluster. Support of the Own/SingleNamespace install modes is implemented into OLMv1 strictly for backwards compatibility with OLMv0 and to allow some limited extra isolation between bundles.

- **multi-namespace watch**\
Only single/own watch namespace modes will be supported. There will not be support for watching multiple namespaces (aside from the default AllNamespaces) mode.

- **No changes to OLM cache infrastructure**\
OLM will continue to watch bundle contents on the cluster-scope, requiring Cluster-Level
list, watch verbs even for Single- or OwnNamespace mode deployed bundles. \
\
This non-goal considerably limits feature complexity and can be improved in future versions of OLMv1.

- **first-class support for switching install modes**\
We won’t stop users from changing the extensions install mode by using the new API. However, OLMv1 won’t go beyond applying the newly generated resources, and removing any resource from the previous installation mode no longer found in the resources for the new installation mode.
