...
- NRI plugins run as privileged pods with host networking and mount the NRI socket from the host. This is inherent to how NRI works—plugins need direct access to the runtime socket—but it means a compromised or buggy plugin has full node-level access. Mitigation: `NriPlugin` is a cluster-scoped CR so only cluster admins can deploy plugins, and the operator creates per-plugin SecurityContextConstraints to keep capability grants to the minimum the plugin actually needs.
...
The plugin hooks into `ValidateContainerAdjustment`, which runs after CRI-O merges adjustments from all NRI plugins. At this stage the full combined adjustment is visible, so the plugin can evaluate the complete set of mutations against namespace policy.
...
- Policy changes require a new MachineConfig rollout—MCO drains and reboots nodes one at a time. This applies to both initial setup and any subsequent policy updates during the cluster's lifetime. The plugin strips disallowed mutations and logs what was stripped, so operators can tune the `allowedMutations` list and observe the effect before tightening policy.
