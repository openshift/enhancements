...
newer inherit the binary from the 4.18 repos, so it is effectively stale and not enabled.
...
monitor will reduce
per-node resource overhead, improve maintainability, and provide a foundation for future pod-level features such as
...
- As a cluster administrator, I want the ability to fall back to the traditional conmon during the transition period so
  ...
  feature set. After GA, fallback requires a MachineConfig override; see
  [Reverting to conmon](#reverting-to-conmon).)

### Goals

1. Establish regular conmon-rs builds in the Node Layer, version-aligned to OCP releases, starting with OCP 5.0.

2. Allow cluster administrators to opt in to conmon-rs via the `ConmonRS` feature gate during Tech Preview.

3. Make conmon-rs the default container monitor in CRI-O for GA, replacing the per-container conmon model with the
   pod-level monitor.
...
2. **Feature Gate**: Introduce a `ConmonRS` feature gate in `TechPreviewNoUpgrade`. When the gate is enabled, the MCO
   ...
   gate is promoted to the `Default` feature set and conmon-rs becomes the default container monitor for all clusters.
...
1. Once the `ConmonRS` feature gate is promoted to the `Default` feature set, the MCO generates the conmon-rs CRI-O
   drop-in for all clusters by default.

2. No administrator action is required. Nodes receive the updated configuration through the standard MCO upgrade
   process.
...
option), which is not available with the traditional conmon. This capability requires conmon-rs >= v0.7.0 and can be
enabled separately once conmon-rs is the active monitor. WebSocket-based streaming is a future benefit of adopting
conmon-rs but is not part of the initial enablement scope (see Non-Goals).
...
At GA, the `ConmonRS` feature gate is promoted to the `Default` feature set, so the MCO generates the conmon-rs drop-in
for all clusters automatically.
...
- The package version should be conmon-rs >= 1.0.0 for the OCP 5.0 release.
...
**Risk**: After GA, there is no API-level toggle to revert to conmon. The feature gate is in the `Default` set and
cannot be individually disabled. Reverting requires either a version downgrade or a manual MachineConfig override.

**Mitigation**: This is an intentional tradeoff to avoid permanent API debt from a transitional field. The Tech Preview
phase provides time to validate conmon-rs before it becomes the default. The MachineConfig override path (setting
`runtime_type = "oci"` on the default handler) provides an emergency escape hatch without requiring a version
downgrade. The conmon binary remains in the node image through at least two GA releases.
...
- Upgrade a cluster from a version without conmon-rs enabled to one where it is the default. Verify that existing
  workloads survive the upgrade and new pods use conmon-rs after node reboot.
...
- Verify that CRI-O exposes container operation metrics (create, start, exec latency and error rates) when conmon-rs is
  the active monitor. Confirm that the metrics are scrapable by the in-cluster monitoring stack.
...
### Tech Preview (Target: OCP 5.0)

- [ ] Enhancement proposal approved
- [ ] conmon-rs >= v1.0.0 binary shipped in the Node Layer, version-aligned to OCP
...
- [ ] `ConmonRS` feature gate promoted to `Default` feature set (conmon-rs is the default container monitor for all
  new clusters and upgrades)
...
During an upgrade to an OCP version where conmon-rs is the default (feature gate in `Default` set), the MCO generates
the conmon-rs CRI-O drop-in automatically. Nodes are drained and rebooted through the standard MCO upgrade process. No
...
conmon-rs is a node-local component. During an upgrade, nodes are updated sequentially through the MCO's rolling update
process. At any point during the upgrade:

- Some nodes may use conmon (pre-upgrade).
- Some nodes may use conmon-rs (post-upgrade).
...
If conmon-rs causes issues during Tech Preview, disable the `TechPreviewNoUpgrade` feature set. After GA, reverting
requires a cluster version downgrade to a release where the `ConmonRS` feature gate is not in the `Default` set, or a
targeted CRI-O drop-in override via MachineConfig that sets `runtime_type = "oci"` on the default handler as a
