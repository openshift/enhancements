...
The MCO, given the previous `irreconcilableMCValidationOverrides` example, 
...
To compute the difference between the install-time parameters and what the 
MCD applies the MCD reads the `/etc/machine-config-daemon/originalconfig` file 
that the MCD creates in its first boot by dumping to that location the 
rendered configuration applied by the first boot. The generation of this file 
by each MCD is part of the scope of this enhancement.
...
- Creation of the `originalconfig` file in each node at first boot time. This
file is a simple dump of the rendered configuration observed at that point.
...
`irreconcilableMCValidationOverrides` configuration is removed, further 
