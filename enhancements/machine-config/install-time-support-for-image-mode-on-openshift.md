...
6. The MCD first boot service detects the different `OSImageURL` value and uses `rpm-ostree rebase` to rebase the node to the custom pre-built container image
7. After the cluster bootstrap completes, MCO starts and begins post-bootstrap processing
...
5. MCO creates a synthetic MachineOSBuild with:
...
- Two boots will occur: Initial disk image boot with stock RHCOS image and a second boot when the MCD first boot service runs to boot into the custom OS container image
