...
`ControlPlaneMachineSets` are singleton Machine API objects that are used to manage control-plane machines. Control plane nodes are not typically scaled in a standard OCP cluster post installation. However, in Openshift's managed offerings(ROSA, ARO etc), there is a not uncommon [need for SREs to resize or replace control plane nodes](https://issues.redhat.com/browse/MCO-1007?focusedId=27526241&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-27526241), and they will often run into pivoting issues described earlier in this EP. By taking over management of these objects, the MCO can ease some pain here. 
