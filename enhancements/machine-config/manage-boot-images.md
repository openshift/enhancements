...
- secure boot & kernel certs [[1](https://redhat.atlassian.net/browse/OCPBUGS-81559)],[[2](https://redhat.atlassian.net/browse/RHEL-129223)],[[3](https://redhat.atlassian.net/browse/COS-4048)]
...
For the following platforms, the boot image update process was automated via the MCO's boot image controller:
| Platform | TechPreview (opt-in) | GA (opt-in) | Default-On (opt-out) |
| -------- | ------- | ------- | ------- |
| gcp*     | 4.16    | 4.17    | 4.19    |
| aws**    | 4.17    | 4.18    | 4.19    |
| vsphere  | 4.20    | 4.21    | 4.22    |
| azure    | 4.21    | 4.21    | 4.22    |
...
The skew enforcement mechanism was developed in parallel to the above timeline, in a platform agnostic manner:
|  | DevPreview      | TechPreview      | GA      |
| -------- | ------- | ------- | ------- |
| skew management | 4.20   |4.21        |4.22        |


##### OSStreams and Managing Boot Images

The OSStream feature allows the user to configure a RHEL stream on a per MachineConfigPool basis. This is critical for users to test a new version of RHEL prior to migrating their whole cluster to a new RHEL major version. One unfortunate wrinkle is that there isn't a 1:1 relationship between MachineSets and MachineConfigPools, as the stub Ignition within the MachineSet determines the pool a newly scaled up machine would join. This doesn't allow for a clear indication as to what stream a MachineSet belongs to, without unmarshalling and decoding the stub Ignition on every sync. To improve stream management for boot images, the following changes were implemented:
- The installer [was updated](https://github.com/openshift/installer/pull/10426) to inject a label, `machineconfiguration.openshift.io/osstream`, designating the OS stream of the MachineSet during cluster installation.
- The MCO's boot image controller [was updated](https://github.com/openshift/machine-config-operator/pull/5752) to check the stream label before performing reconciliation. A MachineSet with no label would be considered the default RHEL stream for that release for backwards compatibility.
- The user is expected to define this label if using a custom RHCOS stream for their user-defined MachineSet.
