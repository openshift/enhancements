...
* Automatic deployment of WASP agent and its machine-level configuration.
* Ease the WASP agent upgrade process i.e. bugfix delivery via z-stream.
* Limit the usage of swap only for OCP Virt users in phase 3.

MCO will make sure that only swap operator will be eligible to configure swap via the KubeletConfig.
This will enable Openshift to opt-in for the NodeSwap FG in its earlier stage before it will be GAed in upstream.
Operator API and its graduation from CNV to OLM will be discussed in a dedicated proposal.
...
3. The cluster admin is enabling swap on Openshift according to the
...
1. The cluster admin is deploying a bare-metal OpenShift cluster.
2. The cluster admin is deploying the OpenShift Virtualization Operator. 
3. The cluster admin is configuring the swap operator.
4. The cluster admin is configuring OpenShift Virtualization for VM memory overcommit via:
