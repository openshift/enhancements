...
6. **migrationController** migrates to remote-key-B → **`encryption.apiserver.operator.openshift.io/migrated-remote-key-id = remote-key-B`**.
...
sequenceDiagram
    ...
    MigCtrl->>SVM: first-enablement write-key=keyName
