...
Although hosted cluster migration to multi-arch is not triggered by "oc upgrade --multi-arch", `ClusterVersion.status` and `ClusterOperator.status` are still populated there the same way as they are on a standalone cluster. Thus, the proposal in this enhancement can be applied to a hosted cluster too.

In addition, the following status can be exploited for the progress of an upgrade:
...
A difference from a standalone cluster is that the image in `HostedCluster.spec.release` does not need to be with the same version number to migrate to multi-arch. (It is not clear for `NodePool.spec.release`.)
