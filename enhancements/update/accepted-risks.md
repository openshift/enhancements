...
The result of `--accept` in the above command is to set up values for the field `.spec.desiredUpdate.accept` on the `clusterversion/version` manifest:
...
> oc adm upgrade accept DualStackNeedsController,LeakedMachineConfigBlocksMCO,OldBootImagesPodmanMissingAuthFlag
 ...
 whose only result is to set up values for the field `.spec.desiredUpdate.accept` on the `clusterversion/version` manifest:
...
The `--append` in the following following command is to append `RiskA` and `RiskB` to the current accept risks (instead of replacing by default):

> oc adm upgrade accept --append=true RiskA,RiskB
