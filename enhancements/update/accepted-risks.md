...
If the cluster administrator decides that those risks `DualStackNeedsController`, `OldBootImagesPodmanMissingAuthFlag`, and `LeakedMachineConfigBlocksMCO` are acceptable and tells the cluster-version operator to accept an update if all its associated risks are among those three (otherwise block it), then a command can be issued to the cluster:

```console
$ oc adm upgrade accept DualStackNeedsController,LeakedMachineConfigBlocksMCO,OldBootImagesPodmanMissingAuthFlag
```

The result of the above command is to set up values for the field `.spec.desiredUpdate.accept` on the `clusterversion/version` manifest:
...
Then the cluster administrator updates the cluster by 

> oc adm upgrade --to 4.18.16
...
The risk name like `OldBootImagesPodmanMissingAuthFlag` is unique and thus the growing accepted risks over time should not cause any problems. However, for any reason, if it is desired to clear them out, we may run the following command:

> oc adm upgrade accept

No risks are provided to the command means that no risks are accepted.

The `--append` in the following following command is to append `RiskA` and `RiskB` to the current accept risks:

> oc adm upgrade accept --append RiskA,RiskB

Note that we have to modify the `patchDesiredUpdate`(https://github.com/openshift/oc/blob/f9d98d644110d3413dc4862002395d0c6dfc1da7/pkg/cli/admin/upgrade/upgrade.go#L682-L696) function so that it does not clobber the existing `accepted` property.

### API Extensions

This enhancement 
- adds a new field 'clusterversion.spec.desiredUpdate.accept': It contains
...
spec:
  ...
  desiredUpdate:
    ...
    version: 4.18.15
status:
  conditionalUpdatesRisks:  # include every risk in the conditional updates (moved up and renamed)
