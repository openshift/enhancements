...
- Work with ART to adjust how they determine if the RHCOS images are up to date.
  Currently, they expect OCP content in those images, but that content will have
  moved to the node image, so they would additionally need to query that to know
  if to trigger a node image build.
