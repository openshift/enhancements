...
Cluster Autoscaler will integrate with Cluster API on OpenShift. This enhancement
...
* No regression of features whether using OpenShift with Machine API or
  Cluster API, regardless of topology (e.g. standalone or hosted control planes).
...
for resource with the `machine.openshift.io` group, depending on the configured
`scaleTargetRef` API group.
...
Cluster Autoscaler Operator will know by the `spec.scaleTargetRef.apiVersion` field whether to look
...
To further prevent users from misconfiguring MachineAutoscalers or MachineSets, a validating
admission policy will be installed to prevent a MachineSet from changing its authoritative
ownership (`.spec.authoritativeAPI`) while a MachineAutoscaler references that MachineSet. This
will help ensure that no MachineAutoscaler is referencing the wrong scalable resource.
...
Due to the synchroniztion that occurs from authoritative resources to their non-authoritative
copies, the Cluster Autoscaler Operator must only write to the authoritative resource.
This places a slightly larger responsibility on the user to specify the correct resource, but
ultimately keeps the functionality predictable with no "magic" connections happening when
the user specifies the wrong resource. The addition of the `status` field on MachineAutoscalers
will help users to solve issues that arise and follows Kubernetes best practices.
