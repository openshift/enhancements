...
**Infrastructure Topology Transition** — A directed change of infrastructure topology independent of control-plane topology. This applies to clusters with dedicated worker nodes where infrastructure workloads are running in SingleReplica mode. The transition moves infrastructure workloads to HA replica counts without modifying the control plane or etcd.
...
2. **Standalone infrastructure topology transition** (SingleReplica → HA with dedicated workers): A new `infrastructureTopology` field in the infrastructure spec expresses the administrator's intent to transition infrastructure workloads independently of the control plane. This applies to clusters with dedicated worker nodes. This path does not modify the control plane or etcd.
...
7. The topology transition controller in CCO detects the `controlPlaneTopology` change and validates preconditions:
   ...
   - No infrastructure topology transition is in progress (the `TopologyTransitionControllerProgressing` condition on the CCO `ClusterOperator` does not carry an `InfrastructureTopologyTransition*` reason) — control-plane and infrastructure transitions are mutually exclusive
...
9. The controller updates the infrastructure status and spec fields:
   ...
   - If `spec.infrastructureTopology` is unset, the controller explicitly sets `spec.infrastructureTopology = HighlyAvailable`. This makes infrastructure topology intent explicit rather than derived.
...
2. The cluster administrator runs `oc adm transition topology --infrastructure HighlyAvailable`
3. The CLI discovers available transitions from the Infrastructure status, validates client-side preconditions (feature gate enabled, no transition in progress), and patches `spec.infrastructureTopology: HighlyAvailable`
4. The API server validates `infrastructureTopology` against the `DesiredInfrastructureTopologyMode` enum
...
5. The topology transition controller in CCO detects the `spec.infrastructureTopology` divergence from `status.infrastructureTopology` and validates preconditions:
   - No control-plane topology transition is in progress (the `TopologyTransitionControllerProgressing` condition on the CCO `ClusterOperator` does not carry a `TopologyTransitionInProgress` reason) — infrastructure and control-plane transitions are mutually exclusive
   - Every ClusterOperator other than cluster-config-operator itself reports `Available=True`, `Progressing=False`, `Degraded=False`
   ...
   The infrastructure controller does not perform cross-field validation against `spec.controlPlaneTopology`. It validates its own prerequisites — worker node readiness, platform support, no conflicting transition — and either admits or rejects based on those. See [No cross-field validation](#infrastructure-api-changes) for rationale.
...
- **Before admission**: if a precondition fails (e.g., insufficient workers, unsupported platform, conflicting CP transition in progress, incompatible upgrade in progress), the `Progressing`/`Upgradeable` conditions carry a diagnostic reason (e.g. `PreflightCheckFailed`, `UnsupportedPlatform`, `UpgradeInProgress`). The controller re-evaluates on every sync — if the prerequisite is later satisfied (e.g., workers become Ready), the transition proceeds automatically.
...
**Withdrawing intent**: Cancellation is not a dedicated feature — it is an emergent property of the spec/status pattern. If, before `status.infrastructureTopology` has been updated, the administrator resets `spec.infrastructureTopology` to match the current status (or clears it to empty), the controller sees no divergence and goes idle. After the status has been updated, the transition is effectively complete. No dedicated CLI cancel command or `--cancel` flag is needed. The administrator can use a direct patch if they need to withdraw intent:
```bash
oc patch infrastructure cluster --type merge -p '{"spec":{"infrastructureTopology":""}}'
```
...
- The transition MUST NOT add, remove, resize, or reconfigure control-plane nodes
...
// DesiredInfrastructureTopologyMode restricts the set of topology modes that can be
// requested as an infrastructure transition target.
// +kubebuilder:validation:Enum=SingleReplica;HighlyAvailable
type DesiredInfrastructureTopologyMode string

const (
	DesiredInfrastructureSingleReplica   DesiredInfrastructureTopologyMode = "SingleReplica"
	DesiredInfrastructureHighlyAvailable DesiredInfrastructureTopologyMode = "HighlyAvailable"
)
...
type InfrastructureSpec struct {
	...
	// InfrastructureTopology expresses the administrator's intent for the
	// cluster's infrastructure topology — how infrastructure workloads are
	// distributed across nodes. Empty by default — the field is unset until
	// an administrator explicitly initiates a transition or the control-plane
	// transition path sets it on compact clusters. When set and the value
	// differs from status.infrastructureTopology, the topology transition
	// controller in cluster-config-operator initiates a transition.
	// An empty value means no transition has been requested.
	// +optional
	// +openshift:enable:FeatureGate=MutableTopology
	InfrastructureTopology DesiredInfrastructureTopologyMode `json:"infrastructureTopology,omitempty"`
}
```
...
The `infrastructureTopology` field follows the same semantics: empty by default, the installer does not populate it. After a successful infrastructure transition, the field retains its value and matches `status.infrastructureTopology`. An empty value means "no infrastructure transition requested."

The `DesiredControlPlaneTopologyMode` and `DesiredInfrastructureTopologyMode` named types restrict accepted values to topology modes that have defined transitions. For the initial implementation, only `SingleReplica` and `HighlyAvailable` are valid for both. Additional values can be added as new transitions are supported.
...
**No cross-field validation**: There is no cross-field validation between `spec.controlPlaneTopology` and `spec.infrastructureTopology`. The infrastructure controller validates its own prerequisites — worker node readiness, platform support, no conflicting transition — and either admits or rejects based on those. On a SingleReplica CP cluster, the control-plane path is responsible for setting `spec.infrastructureTopology` as part of its transition; an administrator would not set it independently on such a cluster, and if they did, the worker node prerequisites would gate admission naturally (a SNO cluster would not have sufficient worker-capable nodes to host HA infrastructure replicas).

**Allowed infrastructure transitions:**

| Source `status.infrastructureTopology` | `spec.infrastructureTopology` | Behavior |
|---|---|---|
| `SingleReplica` | `HighlyAvailable` | **Transition admitted** (prerequisites validated: worker nodes Ready, schedulable, sufficient count) |
...
| Any | empty | No transition requested — controller idle |
...
**Transition progress** will be reported via conditions on the CCO `ClusterOperator` status, following the same pattern for both control-plane and infrastructure transitions. The `Reason`/`Message` fields distinguish infrastructure-specific states:

| Condition Type | Status | Reason | Meaning |
|---|---|---|---|
| `TopologyTransitionControllerProgressing` | `True` | `TopologyTransitionInProgress` | Control-plane transition admitted; CCO is reconciling |
| `TopologyTransitionControllerProgressing` | `True` | `InfrastructureTopologyTransitionInProgress` | Infrastructure transition admitted; CCO is reconciling |
| `TopologyTransitionControllerProgressing` | `False` | `AsExpected` | No active transition |
| `TopologyTransitionControllerProgressing` | `False` | `TopologyTransitionComplete` | Control-plane transition completed successfully |
| `TopologyTransitionControllerProgressing` | `False` | `InfrastructureTopologyTransitionComplete` | Infrastructure transition completed successfully |
| `TopologyTransitionControllerProgressing` | `False` | `PreflightCheckFailed` | Prerequisites not met; transition not admitted |
| `TopologyTransitionControllerProgressing` | `False` | `UnsupportedTransition` | Requested transition is not supported |
| `TopologyTransitionControllerProgressing` | `False` | `UnsupportedPlatform` | Platform does not support the requested transition |
| `TopologyTransitionControllerProgressing` | `False` | `UpgradeInProgress` | Incompatible upgrade blocks the transition |
| `TopologyTransitionControllerUpgradeable` | `False` | `TopologyTransitionInProgress` | Active control-plane transition blocks upgrades |
| `TopologyTransitionControllerUpgradeable` | `False` | `InfrastructureTopologyTransitionInProgress` | Active infrastructure transition blocks upgrades |
| `TopologyTransitionControllerUpgradeable` | `True` | `AsExpected` | No active transition; upgrades allowed |

These reuse the same condition types for both transition paths, distinguished by the `Reason` field prefix (`InfrastructureTopology*` vs. `TopologyTransition*`). This avoids introducing new condition types while keeping the infrastructure path distinguishable.

**Condition semantics:**

- **Progressing=True (InfrastructureTopologyTransitionInProgress)**: The spec/status divergence was detected, prerequisites passed, and CCO is actively reconciling. Operators are expected to react to the upcoming `status.infrastructureTopology` change.
- **Progressing=False (InfrastructureTopologyTransitionComplete)**: The successful terminal state. `status.infrastructureTopology` has been updated to match `spec.infrastructureTopology`. Post-transition soak/validation passed. `controlPlaneTopology` and etcd membership are unchanged.
- **Progressing=False (PreflightCheckFailed)**: A prerequisite is not met. The condition message details which prerequisite failed. The controller re-evaluates on every sync — if the prerequisite is later satisfied (e.g., workers become Ready), the transition proceeds automatically.
- **Upgradeable=False (InfrastructureTopologyTransitionInProgress)**: Set when the infrastructure transition is admitted, cleared when complete. Prevents CVO from starting an upgrade during the transition.

**Failed and interrupted states**: If the transition is admitted but post-transition validation never passes (e.g., an operator fails to reconcile), `Progressing` remains `True` and `Upgradeable` remains `False` indefinitely. The administrator inspects CCO and the relevant operator's logs. The controller continues re-evaluating on every sync. There is no separate "Failed" terminal state.
...
| `spec.infrastructureTopology` | Retains the administrator's declared desired state. Persists across restarts and upgrades. Cleared only if the administrator explicitly resets it. |
| `spec.controlPlaneTopology` | Not modified by the infrastructure transition path. Independent field. |

**Mutual exclusion**: The controller enforces mutual exclusion: only one transition (control-plane or infrastructure) may be in progress at a time. If a control-plane transition is in progress (`Progressing=True` with `TopologyTransitionInProgress` reason), an infrastructure transition is rejected, and vice versa. This is enforced via precondition checks on both paths.

#### Admission Control

**Spec validation**: The `DesiredControlPlaneTopologyMode` named type restricts `spec.controlPlaneTopology` to the set of topology modes that have defined transitions (`SingleReplica`, `HighlyAvailable`). The `DesiredInfrastructureTopologyMode` named type restricts `spec.infrastructureTopology` similarly.
The API server rejects unsupported values at admission time via the kubebuilder enum validation on each type.

**No cross-field admission**: There is no cross-field validation between `spec.controlPlaneTopology` and `spec.infrastructureTopology` at the API admission level. The infrastructure controller validates its own prerequisites at reconciliation time (worker node readiness, platform support, no conflicting transition). See [No cross-field validation](#infrastructure-api-changes) for rationale.
...
- Enforces mutual exclusion: only one transition (control-plane or infrastructure) may be in progress at a time
...
- No infrastructure topology transition is in progress (the `TopologyTransitionControllerProgressing` condition does not carry an `InfrastructureTopologyTransition*` reason)
...
- No control-plane topology transition is in progress (the `TopologyTransitionControllerProgressing` condition does not carry a `TopologyTransitionInProgress` reason)
- Every ClusterOperator other than cluster-config-operator itself reports `Available=True`, `Progressing=False`, `Degraded=False`
...
The infrastructure controller does not perform cross-field validation against `spec.controlPlaneTopology`. It validates its own prerequisites and either admits or rejects based on those.
...
If no supported transition matches, or a precondition fails, the controller records the reason on the `Progressing`/`Upgradeable` conditions (e.g. `PreflightCheckFailed`, `UnsupportedPlatform`, `UpgradeInProgress`), so the administrator can revert `spec.infrastructureTopology` to resolve it.
...
- The controller MUST reject an infrastructure transition if a control-plane transition is in progress, and vice versa

##### Discovery

CCO evaluates the current cluster state and advertises available transitions through the Infrastructure status. For each transition path, the controller populates discovery information that the CLI consumes to present available transitions:

```yaml
status:
  availableTransitions:
    - type: ControlPlaneTopology
      targetTopology: HighlyAvailable
      prerequisites:
        - type: ControlPlaneNodeCount
          status: "True"
        - type: MutableTopologyEnabled
          status: "True"
      available: true
    - type: InfrastructureTopology
      targetTopology: HighlyAvailable
      prerequisites:
        - type: WorkerNodeCount
          status: "True"
          message: "3 worker-capable nodes are Ready and schedulable"
        - type: MutableTopologyEnabled
          status: "True"
        - type: NoPlatformConflict
          status: "True"
        - type: NoConflictingTransition
          status: "True"
      available: true
```

`oc adm transition topology` consumes this discovery information to present available transitions. The CLI does not duplicate server-side validation.

Discovery updates when: node readiness changes, upgrades start/complete, conflicting transitions start/complete, or feature gate state changes.
...
- Displays prerequisites and their status before submission; requires explicit confirmation (or `--yes`), and supports `--allow-transition-with-warnings` for non-blocking prerequisites
...
For infrastructure-only transitions, the CLI supports `oc adm transition topology --infrastructure HighlyAvailable`. The CLI auto-detects available transitions via the discovery contract based on cluster state (worker nodes present, `spec.controlPlaneTopology` already matching status).
...
| --------- | ---------------- |
| cluster-config-operator | Topology transition controller; watches `spec.controlPlaneTopology` and `spec.infrastructureTopology`, coordinates transitions, updates status topology fields, enforces mutual exclusion |
| Infrastructure API (`openshift/api`) | Add `controlPlaneTopology` and `infrastructureTopology` to `InfrastructureSpec` with named types; update immutability documentation on status topology fields |
...
|---|---|---|
| **API contract** (`spec.controlPlaneTopology`, `spec.infrastructureTopology`, named types, conditions) | openshift/api maintainers | Define spec fields, named types with enum validation, feature gate, and condition semantics |
...
- CCO MUST reject an infrastructure transition if a conflicting control-plane transition (`spec.controlPlaneTopology != status.controlPlaneTopology`) is in progress.
- CCO MUST reject a control-plane transition if a conflicting infrastructure transition is in progress (mutual exclusion).
...
- Withdrawing intent is unclear without a spec field to reset. The controller would need to invent its own cancellation mechanism.
...
8. **Discovery contract shape**: The `status.availableTransitions` discovery field must be agreed with API reviewers. This includes the structure of prerequisites, whether to use a list of conditions or a summary boolean, and how the CLI consumes this information.
...
| Infra precondition validation | Verify the controller withholds admission when workers are insufficient/not-ready, unsupported platform, conflicting CP transition in progress, or incompatible upgrade in progress |
| No cross-field validation | Verify that the infrastructure controller does not perform cross-field validation against `spec.controlPlaneTopology` — it validates its own prerequisites independently |
| Mutual exclusion | Verify simultaneous CP and infrastructure transition requests are rejected |
...
| Intent withdrawal | Verify that clearing or resetting spec before status update causes the controller to go idle |
...
- Concurrent operation testing: CP transition + infra transition attempt (verify mutual exclusion)
...
- `DesiredInfrastructureTopologyMode` named type validated in API integration tests
- Standalone infrastructure transition controller path implemented with mutual exclusion
...
- Mutual exclusion validated end-to-end
...
On upgrade to a version with `MutableTopology` support, `spec.controlPlaneTopology` and `spec.infrastructureTopology` are empty (zero value for optional strings). The controller interprets empty as "no transition requested" and takes no action. No migration needed.
...
The `DesiredControlPlaneTopologyMode` and `DesiredInfrastructureTopologyMode` named types provide API-server-level validation with no additional services required. Topology status fields are not protected by admission policies — this is consistent with other infrastructure status fields.
...
- Infrastructure CR API changes (`DesiredControlPlaneTopologyMode`, `DesiredInfrastructureTopologyMode` types, `controlPlaneTopology` and `infrastructureTopology` spec fields)
