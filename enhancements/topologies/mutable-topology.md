...
// ControlPlaneTopologyTransitionStatus expresses the last observed state of the control-plane portion of the topology transition.
...
// InfrastructureTopologyTransitionStatus reports the transition state for the infrastructure topology dimension.
// +kubebuilder:default=Idle
// +kubebuilder:validation:Enum=Idle,Pending,Error,RetryWithBackoff,Transitioned
// +optional
...
The singular `topologyTransitionStatus` field is replaced by the two dimension-specific fields above. This is an intentional API shape change for the feature-gated enhancement; API and generated-resource implementation must account for the field rename before the feature is enabled for users.

**Transition progress** will be reported via the paired topology transition status fields of the `infrastructures.config.openshift.io` API object called "cluster".
The paired status values are used to derive the `reason` and `status` of both the `Progressing` and `Upgradeable` conditions on the CCO `ClusterOperator` (reported internally by the controller as `TopologyTransitionControllerProgressing` and `TopologyTransitionControllerUpgradeable`, which library-go's status-sync mirrors onto `Progressing` and `Upgradeable` respectively). The paired status fields are authoritative and must remain equal for the initial coupled transition — the two conditions exist because CVO's upgrade gating and general operator tooling both key off the standard ClusterOperator condition contract, not the Infrastructure object.

The two fields are status dimensions of one coupled transition in this initial implementation, not independent state machines. `controlPlaneTopologyTransitionStatus` describes the control-plane portion and `infrastructureTopologyTransitionStatus` describes the infrastructure portion; because there is no separate infrastructure topology request, both values are written together and must remain equal. If they diverge, the controller must not declare the transition complete: it should keep upgrades blocked, report the inconsistency as degraded, and reconcile the status back to a consistent state.

The following table enumerates the possible values of the paired topology transition status fields and what they represent, along with the `Progressing`/`Upgradeable` condition state each value drives.
...
| `RetryWithBackoff` | The controller encountered an actual sync or API error while reconciling the requested transition. The controller returns the error, the standard rate-limited workqueue retries with backoff, and the controller reports `Degraded=True`. This state is not used for unmet transition preconditions or expected downstream reconciliation. | `status: True`, `reason: TopologyTransitionRetrying` | `status: False`, `reason: TopologyTransitionRetrying` |
...
| -------------- | ------- |
| `Progressing` | Whether the controller is actively reconciling a topology transition. `status: True` while both topology transition status fields are `Pending` or `RetryWithBackoff`; `status: False` when idle, in error, or complete. |
| `Upgradeable` | Whether CVO may start an upgrade. `status: False` while a transition is requested, pending, in progress, or being retried; `status: True` when idle or complete. |

`Progressing` and `Upgradeable` are existing operator conditions on the CCO `ClusterOperator` (internally named `TopologyTransitionControllerProgressing` and `TopologyTransitionControllerUpgradeable` by the controller, unioned onto `Progressing` and `Upgradeable` by library-go's status-sync), not dedicated transition condition types — the `Reason`/`Message` fields distinguish states (e.g. `UnsupportedTransition`, `PreflightCheckFailed`, `TopologyTransitionInProgress`, `TopologyTransitionRetrying`, `TopologyTransitionComplete`, `AsExpected`). For `RetryWithBackoff`, the controller also reports its standard controller-specific `Degraded=True` condition while the sync/API error is being retried.
...
**Status fields**: The existing topology status fields (`controlPlaneTopology`, `infrastructureTopology`, `controlPlaneTopologyTransitionStatus` and `infrastructureTopologyTransitionStatus`) are not protected by admission policies. This is consistent with other infrastructure status fields — no special protection exists for them today. An administrator who deliberately modifies these values outside the transition controller does so at their own risk.
...
2. Prioritize the condition writes: set `Progressing=True` and `Upgradeable=False` (`reason: TopologyTransitionInProgress` for both) on the CCO `ClusterOperator` before publishing the Infrastructure transition status, so CVO cannot start an upgrade while the transition is being applied
3. Update `status.controlPlaneTopology`, `status.infrastructureTopology`, and `status.controlPlaneTopologyTransitionStatus` and `status.infrastructureTopologyTransitionStatus` (set to `Pending`) together in a single Infrastructure status update. The paired topology transition status fields are the authoritative record that a transition is in progress — the controller reads these fields back on later syncs rather than re-deriving its own state from ClusterOperator conditions. The two writes are not atomic; if the condition update succeeds but the Infrastructure status update fails, the sync returns an API error and the standard rate-limited workqueue retries while `spec.controlPlaneTopology` remains different from `status.controlPlaneTopology`.
...
If no supported transition matches, or a precondition fails, the controller sets `status.controlPlaneTopologyTransitionStatus` and `status.infrastructureTopologyTransitionStatus` to `Error`, posts a Warning Event on the Infrastructure object, and derives matching `Progressing=False`/`Upgradeable=False` reasons on the ClusterOperator. It returns successfully without requesting a rate-limited retry; a later relevant informer event causes the request to be evaluated again. Actual sync/API errors are handled separately through `RetryWithBackoff` and the standard rate-limited retry queue.
...
Once all criteria pass, the controller sets `status.controlPlaneTopologyTransitionStatus` and `status.infrastructureTopologyTransitionStatus` to `Transitioned` and derives `Progressing=False` and `Upgradeable=True` on the ClusterOperator.
...
| ------------ | ------ | -------- |
| Admission precondition is false | No impact — transition not admitted; `controlPlaneTopologyTransitionStatus=Error` and `infrastructureTopologyTransitionStatus=Error`; a Warning Event identifies the reason | Address the precondition; a relevant cluster-object change triggers reevaluation |
| Sync/API error during reconciliation | Transition progress is interrupted; the controller reports `RetryWithBackoff` and `Degraded=True` | Inspect the error; standard rate-limited workqueue backoff retries the sync |
