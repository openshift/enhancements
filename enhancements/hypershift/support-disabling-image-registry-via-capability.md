...
`HostedCluster` grows an optional array field `disabledCapabilities` in its `spec`,
indicating what components users want to disable at install time.

Initially, the only supported value in this array is `ImageRegistry`.

Editing this field at runtime is only allowed if the user is removing
capabilities from the array. Adding capabilities to the array is not supported.
...
The `CVO` reconciler edits the `ClusterVersion` resource, setting
...
TODO: does the `hypershift` cli needs to change?

### Workflow Description

1. Cluster service consumer creates a HostedCluster object to be handled by
Hypershift, setting `.spec.disabledCapabilities` to `["ImageRegistry"]`, the
...
Registry operator, updating the `ClusterVersion` resource accordingly
...
const ImageRegistryCapability OptionalCapability = "ImageRegistry"
