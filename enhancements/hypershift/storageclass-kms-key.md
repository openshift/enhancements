...
This enhancement adds an optional `initialKMSKeyARN` field under
...
This enhancement adds an optional `initialKMSKeyARN` field to the HostedCluster API under
...
When `initialKMSKeyARN` is set, the CPOv2 component framework blocks the CSO from starting
...
1. The cluster administrator runs:
   ...
   hcp create cluster aws \
     ...
     ...
   ```
   or sets `spec.operatorConfiguration.csiDriverConfig.aws.initialKMSKeyARN` directly on the
   `HostedCluster` manifest.
2. The HostedCluster controller creates the `HostedControlPlane` with `operatorConfiguration`
   mirrored.
3. The CPO reconciles the `HostedControlPlane`, sees that `initialKMSKeyARN` is set,
...
#### New field: `initialKMSKeyARN` on `AWSCSIDriverConfig`
...
`infraID`/`clusterID` precedents: `initialKMSKeyARN` is immutable once set, cannot be
...
- Initial creation with `initialKMSKeyARN`: HCCO writes the field.
- Initial creation without `initialKMSKeyARN`: HCCO leaves `DriverConfig` unset.
...
- Upgrade of a cluster that never had `initialKMSKeyARN`: the resource already exists,
  `DriverConfig` is untouched.

#### Bootstrap Ordering: CSO Conditional Dependency on HCCO

When `initialKMSKeyARN` is set, the HCCO must write the KMS key to `ClusterCSIDriver`
...
2. The CSO component (`cluster-storage-operator`) conditionally declares the HCCO
   as a dependency when `initialKMSKeyARN` is set in the HostedControlPlane spec. The CPOv2 framework
...
The resulting bootstrap sequence when `initialKMSKeyARN` is set:
...
When `initialKMSKeyARN` is not set, the CSO has no dependency on the HCCO and starts
...
onUpdate:
- Changing `initialKMSKeyARN` value is rejected
- Removing `initialKMSKeyARN` is rejected
- Removing `aws` while `initialKMSKeyARN` was set is rejected
- Removing `csiDriverConfig` while it was set is rejected
- Adding `initialKMSKeyARN` to a cluster created without it is rejected
- Removing `operatorConfiguration` while `initialKMSKeyARN` was set is rejected
...
1. **Day-1 key configuration:**
   - Create a `HostedCluster` with `operatorConfiguration.csiDriverConfig.aws.initialKMSKeyARN`
     set
...
3. **Regression (no key configured):**
   - On a hosted cluster where `initialKMSKeyARN` was never set, verify PVCs use
...
HyperShift. If an N->N+1 upgrade fails mid-way, the `initialKMSKeyARN` field is not yet
active and has no effect on storage behavior. If `initialKMSKeyARN` was already configured
...
If `initialKMSKeyARN` is set on a HostedCluster running a guest release that predates this
