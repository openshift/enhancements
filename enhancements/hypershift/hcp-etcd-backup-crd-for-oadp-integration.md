...
This enhancement introduces a new `HCPEtcdBackup` CRD in the `hypershift.openshift.io/v1beta1` API group that serves as the contract between the OADP HyperShift plugin and the Hypershift Operator for triggering etcd backups and discovering backup URLs. A new controller in the Hypershift Operator watches `HCPEtcdBackup` resources, orchestrates snapshot and upload Jobs in the HO namespace, and reports results back through the CR status. This design keeps management-level cloud credentials isolated from HCP namespaces and **eliminates the need to pause HostedCluster/NodePool reconciliation during backups** — the previous CSI volume snapshot approach required pausing reconciliation for 20-30 minutes per backup cycle, blocking all day-two operations. With this enhancement, backup Jobs run independently without interfering with cluster controllers.
...
5. Support optional per-tenant KMS encryption of backup artifacts in cloud storage via keys defined at the HostedCluster level.
...
- **[CNTRLPLANE-2676](https://issues.redhat.com/browse/CNTRLPLANE-2676) — HCPEtcdBackup CRD for OADP Integration (self-managed)**: Covers the core CRD API, controller, CPO subcommands (`fetch-etcd-certs`, `etcd-backup`, `etcd-upload`), OADP plugin integration, E2E tests, envtest validation, and Tech Preview documentation. **Status: Dev Complete** — all child issues closed and PRs merged.

- **[CNTRLPLANE-3312](https://issues.redhat.com/browse/CNTRLPLANE-3312) — HCPEtcdBackup support for ROSA and ARO**: Extends the feature to managed platforms (ROSA HCP / ARO HCP) with credential auto-detection for IRSA/STS (AWS) and Workload Identity (Azure), GA feature gate promotion, and managed-platform E2E validation. **Status: In Progress** — credential auto-detection ([CNTRLPLANE-3167](https://issues.redhat.com/browse/CNTRLPLANE-3167)) is under review, GA promotion pending.
...
2. The `HCPEtcdBackupReconciler` in the Hypershift Operator detects the new CR and:
...
sequenceDiagram
    ...
    OADP->>CR: 1. Create HCPEtcdBackup CR
...
// HostedControlPlane and upload it to cloud storage. Creating this
// resource triggers the backup workflow.
type HCPEtcdBackup struct {
    metav1.TypeMeta   `json:",inline"`
    metav1.ObjectMeta `json:"metadata,omitempty"`
    Spec   HCPEtcdBackupSpec   `json:"spec,omitempty"`
    Status HCPEtcdBackupStatus `json:"status,omitempty"`
}
...
type HCPEtcdBackupStatus struct {
    ...
    // +optional
    EncryptionMetadata *HCPEtcdBackupEncryptionMetadata `json:"encryptionMetadata,omitempty"`
}
...
**Controller predicate**: The `HCPEtcdBackupReconciler` includes a feature gate predicate check at the start of `Reconcile()`. If the `HCPEtcdBackup` feature gate is not enabled, the reconciler is a **no-op** — it returns immediately without processing the CR. This ensures that even if an `HCPEtcdBackup` CR is created while the gate is disabled, no backup Job or NetworkPolicy is created.
...
The backup Job uses a fixed ServiceAccount named `etcd-backup-job` in the HO namespace. This ServiceAccount is created and managed by the `HCPEtcdBackupReconciler` with permissions to read Secrets/ConfigMaps from HCP namespaces (for the InitContainer to fetch etcd TLS certificates) and write access to cloud storage (S3 `PutObject`, Azure Blob write).

The SA name is fixed by design. The SRE/infra team controls the effective permissions by binding their IAM role or federated credential to this known SA name. The controller handles platform-specific annotations automatically (e.g., `azure.workload.identity/client-id` for Azure Workload Identity). Making the SA name configurable was considered but deferred — it would add coordination complexity (pre-create SA, configure IAM bindings, communicate the name) without clear benefit, since the permission control point is the IAM role binding, not the SA name itself. These are infrastructure-level changes that require manual provisioning and cannot be changed at runtime.
...
The controller supports configurable `MaxConcurrentReconciles`, allowing parallel backup processing when needed. The initial default is serial execution (one backup at a time) as a conservative starting point. Once Velero/OADP supports triggering parallel backups (currently Velero processes backups sequentially), the concurrency can be increased to match. Further optimizations (node affinity, bandwidth throttling) will be evaluated if performance issues arise.
...
Backup retention is managed via TTL and count-based policies configured externally (e.g., through the HO CLI or Velero TTL). The `HCPEtcdBackup` CRs are simple records — deleting a CR does **not** delete the corresponding cloud storage object.
...
- **Overwrite prevention** — For **Azure Blob Storage**, the `etcd-upload` subcommand uses the `IfNoneMatch: ETagAny` conditional write header, which prevents overwriting an existing blob (Azure returns `412 ConditionNotMet`). For **AWS S3**, conditional writes via `If-None-Match` are **not supported** because the upload uses the AWS SDK v2 Transfer Manager, which handles multipart uploads internally and does not expose conditional write headers in its API. Instead, overwrite prevention on S3 is mitigated at the application level: the upload key includes a Unix timestamp (`{keyPrefix}/{timestamp}.db`), making key collisions practically impossible. Combined with **S3 Versioning** on the bucket as an additional safety net, original objects are preserved even in edge cases.
...
The HO CLI (or an external operator) is responsible for cleaning up old `HCPEtcdBackup` CRs based on count thresholds (e.g., keep the last 5 `HCPEtcdBackup` CRs per HCP, delete the oldest Kubernetes object when exceeded). Cloud storage object cleanup is handled by the storage backend's lifecycle policies.
...
Note: Velero is exploring Object Lock support ([velero-io/velero#8686](https://github.com/velero-io/velero/issues/8686)). Since the HCPEtcdBackup snapshot is written to a path within the Velero backup directory (see [OADP Plugin Integration](#oadp-plugin-integration)), future Object Lock enforcement on the bucket will apply to snapshot objects as well. This is compatible with the current design since the controller never deletes S3 objects.
...
The controller automatically detects the credential type from the referenced Secret content at runtime, without requiring any API-level credential type field. This enables transparent support for both self-managed and managed platforms (ROSA HCP, ARO HCP) without configuration changes. The detected modes are:

| Mode | Detection | Credentials File Mounted? |
|------|-----------|--------------------------|
| AWS Static | INI file with `aws_access_key_id` | Yes |
| AWS STS/IRSA | Secret value starts with `arn:` | No — projected SA token + `AWS_ROLE_ARN` env var |
| Azure Client Secret | JSON with non-empty `clientSecret` | Yes |
| Azure Workload Identity | Key `cloud` with `AZURE_CLIENT_ID` | No — webhook injects federated token |
| Azure Managed Identity | Key `cloud` without `AZURE_CLIENT_ID` | Yes |

For AWS STS, the backup Job includes a projected ServiceAccount token volume with `sts.amazonaws.com` audience and 1-hour expiry. For Azure Workload Identity, the controller sets the `azure.workload.identity/client-id` annotation on the `etcd-backup-job` ServiceAccount and adds the `azure.workload.identity/use: "true"` label to the pod template, allowing the Azure WI webhook to inject the federated token at runtime.
...
2. Maps the BSL to `HCPEtcdBackupStorage` (S3 or AzureBlob) and resolves credentials (explicit BSL credential ref, or fallback to `cloud-credentials` secret for standalone Velero).
3. Copies the credential Secret to the HO namespace.
4. Creates the `HCPEtcdBackup` CR in the HCP namespace and polls for completion.
5. On success, stores the `snapshotURL` as an annotation on the HostedControlPlane/HostedCluster objects in the Velero tarball.

**Restore flow:**
1. The plugin's `RestoreItemAction` reads the `snapshotURL` annotation from the backed-up HostedControlPlane/HostedCluster.
2. Generates a presigned URL for the snapshot: S3 presigned GET (with STS `AssumeRoleWithWebIdentity` if needed) or Azure Blob SAS (account-key or AAD User Delegation).
3. Injects the signed URL into `spec.etcd.managed.storage.restoreSnapshotURL`.
4. The existing HyperShift restore machinery picks up the URL and restores etcd from the snapshot.
5. For etcd StatefulSets: the plugin skips PVC restore when the backup method is `etcdSnapshot` (the snapshot replaces the PVCs).

**Upload path:** The snapshot is uploaded to `{keyPrefix}/{timestamp}.db` where `keyPrefix` is constructed by the plugin to fall within Velero's backup directory (`{bsl-prefix}/backups/{backup-name}/etcd-backup/`). This enables Velero's `DeleteBackup` to cascade-delete the snapshot via its `ListObjects` cleanup. This path convention is not documented as a supported Velero interface — the Velero maintainer (@kaovilai) has acknowledged this dependency and committed to coordinating any upstream layout changes.
...
Adding an `HCPEtcdBackupARN` field to `AWSRolesRef` for per-HostedCluster S3 credentials. Rejected because:
...
1. ~~**Service Account for backup Jobs**~~: Resolved — the controller uses a fixed `etcd-backup-job` ServiceAccount created and managed by the reconciler. The SRE/infra team controls permissions via IAM role bindings to this known SA name. Making the SA configurable was deferred as it requires infrastructure-level changes without clear benefit. See [Service Account for Backup Jobs](#service-account-for-backup-jobs).
2. ~~**CRD naming**~~: Resolved — the CRD is named `HCPEtcdBackup` to avoid confusion with standalone etcd backup mechanisms and to clearly indicate it is specific to HostedControlPlanes.
3. ~~**HCPEtcdBackup CR retention policy**~~: Resolved — retention is managed via TTL and count-based policies configured externally (HO CLI, Velero TTL, or cloud storage lifecycle policies). See [Backup Retention](#backup-retention).
4. ~~**Per-HCP artifact encryption (SSE-KMS / Azure CMK)**~~: Resolved — optional per-tenant KMS key fields (`kmsKeyARN` for S3, `encryptionKeyURL` for Azure Blob) are included in the `HCPEtcdBackup` spec and in a new `HCPEtcdBackupConfig` struct on `ManagedEtcdSpec`. The KMS key is defined once at the HostedCluster level (immutable), propagated HC → HCP → HCPEtcdBackup CR spec by the controller. The OADP plugin does not set the KMS key. The backup Job's ServiceAccount must have KMS permissions when a key is configured. AWS SSE-KMS is fully implemented. Azure CMK mechanism is pending ARO team's decision. See [Artifact Encryption (SSE-KMS / Azure CMK)](#artifact-encryption-sse-kms--azure-cmk) and [Extension to ManagedEtcdSpec](#extension-to-managedetcdspec-hostedcluster--hostedcontrolplane).
5. **Azure encryption mechanism**: The specific Azure encryption mechanism (Encryption Scopes with CMK, account-level CMK, or client-side encryption) is pending a decision from the ARO HCP team. The API field and architecture are ready to support any of these approaches with localized changes to the Azure uploader.

## Test Plan

- **Unit tests**: Controller reconcile logic, Job manifest construction, Secret copy/cleanup, status condition updates.
- **Integration tests**: Create an `HCPEtcdBackup` CR, verify the controller creates a Job, simulate Job completion, verify status updates.
...
The `HCPEtcdBackup` CRD is consumed only by the HO controller and the OADP plugin. Both components are versioned independently. The CRD schema provides forward compatibility through optional fields and standard Kubernetes condition conventions.
