...
`LoadBalancer`, which blocks non-cloud platforms (Agent, KubeVirt, None) that lack
...
and `OIDC` — to cover the private router, giving operators explicit control over the
...
router in the HCP namespace. This per-control-plane router serves as the single
ingress point for all control plane routes, including:
...
- **Custom DNS control** — Route publishing allows customers to specify explicit
  hostnames (e.g., `api.mycluster.example.com`) for each control plane endpoint.
  The customer owns the DNS resolution, enabling air-gapped, split-horizon, and
  hybrid cloud DNS configurations.

- **On-premises and bare-metal deployments** — Customers running HyperShift on
  Agent, KubeVirt, or None platforms use Route publishing because `LoadBalancer`
...
- **Multi-tenant isolation** — Each hosted control plane gets its own dedicated
  ...
  multiple hosted clusters for different teams or customers.
...
| New HC on Agent/KubeVirt/None | User sets `Router` with `NodePort` + address |
...
This change is scoped to the management cluster control plane (CPO). It affects:

- **Standalone (self-managed)**: Fully supported — operators set the address to a
  management cluster node IP.
- **HyperShift Hosted**: Supported — the management cluster operator controls the
  service publishing strategy.
- **ARO HCP / ROSA HCP**: Not affected — these always run on cloud platforms with
  LB support and will continue using `LoadBalancer`.
...
- Documentation for non-cloud platform setup

### GA

- E2E validation on Agent and KubeVirt platforms
- Upgrade testing (clusters without `Router` entry continue working)
- At least one release cycle of soak time
