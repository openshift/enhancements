...
### Silent-Ignore Gap: Older CPO Does Not Reconcile New Log Level Fields

The N-1/N+1 compatibility table documents the expected behavior: an older CPO silently
ignores unknown JSON keys in `OperatorConfiguration`. This is correct Go/JSON behavior, but
it creates an observability gap. When an administrator sets a log level field on the
HostedCluster CR, they have no way to determine whether the field was actually reconciled by
the CPO or quietly dropped because the CPO predates `HCPUserFacingOperatorLogs`. The CPO
continues reconciling all other fields it knows about — no error, no degraded condition, no
status update. API consumers (SREs, automation, OCM integrations) may assume their spec is
in effect when it is not.

Explore a compatibility checker between the hypershift API and CPO-supported fields to
surface this skew to administrators.
...
- 2026-09-17: Added TODO section documenting the silent-ignore gap where an older CPO
  silently ignores new log level fields stored in the API without surfacing any feedback
  to administrators; noted direction to explore a compatibility checker between the
  hypershift API and CPO-supported fields
