...
- Recover pools from cluster api provider kubevirt pod restarts or node restarts.
...
3. **CoreOS Afterburn**: Enable parsing and application of OpenStack network config standard data as dracut network kernel args from cloud-init config drive for KubeVirt VMs, this is similar what is done at proxmoxve provider.

### Workflow Description

**cluster administrator** is a human user responsible for deploying a HyperShift hosted cluster.

1. Disable the default pod network and select the network to replace it:
   - Sets `AttachDefaultNetwork=false` in the NodePool specification
   - Specifies an additional network attachment that will replace the default network
   - Ensures the static IP configuration matches the custom network's name

2. The cluster administrator defines an IP pool configuration in the HostedCluster nodepool specification, including:
   - Network name (to assign a ip pool)
   - IP address ranges in CIDR notation, hyphenated format, or as individual IPs
   - Gateway address
   - DNS nameservers

3. The cluster administrator creates the HostedCluster resource with the KubeVirt platform type and network configuration

4. HyperShift reconciles the kubevirt nodepool and create KubevirtMachineTemplate resource for the guest cluster nodes.
...
- **IP Pool Exhaustion**: If all IPs in the pool are allocated, machine creation will fail with a clear error message indicating pool exhaustion
...
- **Network Configuration Failure**: If afterburn cannot parse or apply network configuration, the node bootstrap will fail and the issue will be visible in console logs
...
- New field in KubevirtPlatformSpec:
  ...
  type KubevirtPlatformSpec struct {
      // ... existing fields ...
      BootstrapNetworkConfig *KubevirtBootstrapNetworkConfig
  }
  ```
...
**Improvements**:
- If we manage to merge the [PR](https://github.com/coreos/ignition/pull/2134) to use NoCloud with ignition instead of ConfigDrive
  then the network data format can be cloud init network config v1 or v2 and also JSON or yaml, this is more human friendly than
  current opnstack config format.
...
**Mitigation**: The cluster-api-provider-kubevirt controller use only one gorouting for it, but still the in memory pool need to be protected with read/write locks
...
One option related to the API instead of having a global bootstrap config concept for minimal configuration to allow to retry ignition was to the network configuration as options to the additional networks we pass today to specify the network attachement definition name, this may allow users to configure networking not only for the first interface but for all the secondaries.
...
- Detection: Check cluster-api-provider-kubevirt controller logs
- Log output: IP allocation errors in controller logs
- Metric: `kubevirt_ip_allocation_errors_total`
