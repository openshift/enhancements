...
With this solution IPAM configuration will be done at the ovn-kubernetes localnet user defined network instead of the hypershift kubevirt nodepoo
...
- Contributing this feature to ovn-kubernetes may take months if not years and a lot of effort, 
...
With this solution the addresses will be assigned by ovn-k but the rest of network config will be passed as part of the nodepool

Missing features from current localnet implementation:
- There is no official support for localnet + IPAM

Pro:
- From ovn-kubernetes perspective the only part missing is having official support for localnet + IPAM
- This works for VMs and pods, so it would be possible in the future to also put the control plane under same network and IPAM.
- It will be supported for ovn-kubernetes team, not only hypershift kubevirt maintainers.
- There is a center place to allocate IPs so there will be no overlap between hosted clusters
- Not new code at ovn-kubernetes
- Changes at hypershift are way smaller
...
- Only CIDRs supported, not hyphen formats like 192.168.3.4-192.168.3.7
```
