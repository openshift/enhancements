...
* Add support for chunk verification in `c/storage`
* Add support for lazy images pulling from a private registry in `c/image`
...
In order to lazy pull images, we need to have [Stargz Store plugin](https://github.com/containerd/stargz-snapshotter/blob/aaa46a75dd97e401025f82630c9d3d4e41c9f670/docs/INSTALL.md#install-stargz-store-for-cri-opodman-with-systemd) installed on the node as well as `additionallayerstores` configuration for container storage library. [ContainerRuntimeConfig](https://github.com/openshift/machine-config-operator/blob/release-4.16/docs/ContainerRuntimeConfigDesign.md) will be used to deploy the required configuration on the target nodes.
...
spec:
 ...
 containerRuntimeConfig:
     lazyImagePull:
        enabled: true
```
* Optionally, A list of supported types of the image format for lazy pulling can also be specified. At the time of writing this document, Openshift will only support `zstd:chunked` type image format. If the user doesn't explicitly specify the image format, the default value is assumed to be `zstd:chunked`.
```yaml
apiVersion: machineconfiguration.openshift.io/v1
kind: ContainerRuntimeConfig
metadata:
...
`c/storage` library needs to support verification of the downloaded chunks. This is abosolutely critical for the production setup.

## Add support for lazy images pulling from a private registry in `c/image`

`c/image` needs to pass the required credentials to pull the container image from a private registry to stargz store.
...
The current implementation in `c/storage` doesn't verify chunks on the host, creating a vulnerability for users with compromised registries. To address this issue, chunk verification will be added to `c/storage` before enabling this feature for users.
...
* Chunk verification implemented in `c/storage`
* Drop-in configuration support in `c/storage`
* Support lazy pulling from private registry
...
* Downgrade expectations -  If you downgrade the cluster to a version that does not support the `zstd:chunked` image format, any containers using this format will fail to start. However, since `zstd:chunked` is supported by `c/storage`, deleting the image from the node (for example, using `crictl rmi <image>`) will allow the image to be redownloaded, and the containers should start successfully. Note, though, that without the benefits of lazy image pulling, the start time for these containers will be similar to that of regular container images.
...
Container that used lazy pulled images will fail to start, nodes need to be cleaned up using `crictl rmi <image>` to recover.

#### What consequences does it have on existing, running workloads?

MCO will remove the required configuration in the container storage library on the host and reboot the node. This will lead to container that used lazy pulled images fail to start, nodes need to be cleaned up using `crictl rmi <image>` to recover.

#### What consequences does it have for newly created workloads?

If the workload image was never lazy pulled, then there won't be any impact. But, if it the backing container image was ever lazy pulled then the node will need to be cleaned up using `crictl rmi <image>` to recover.
