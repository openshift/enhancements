...
Platform alerts will be updated via entries in the `AlertRelabelConfig` CR (in `cluster-monitoring-config`) rather than editing their original `PrometheusRule`.

## Motivation
- The current `PrometheusRule` CRD + `oc apply` workflow is powerful yet cumbersome for day‑to‑day alert tweaks.
...
1. **Bulk disable during maintenance**
   ...
   so that I can suppress non‑critical notifications across all namespaces during planned maintenance windows.

2. **Single‑alert toggle from alert details**
   As an SRE on call, I want to click “Disable” or “Enable” on an individual alert’s details page,
...
4. **Create alert from a metric query**
   ...
   so that I can turn any ad‑hoc query I used during troubleshooting into a managed alert.

5. **View silence status on active alerts**
...
| Method  | Path                                                     | Description                                                                                       |New REST API Required                                                                                 |
|---------|----------------------------------------------------------|---------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| GET     | `/alerting/alerts`                                       | List active alerts (Pending / Firing / Silenced) with post‑relabel labels.                        | **Required** - Since we need to add the logic for merging `AlertRelabelConfig` with the plaftform alerts           |
| GET     | `/alerting/rules`                                        | List all alert rule definitions, including `disabled` flags and relabel context.                  | **Required** - Since we need to add the logic for merging `AlertRelabelConfig` with the plaftform alerts           |
| GET     | `/alerting/rules/{alertname}/severity/{severity}`        | Fetch a single rule’s full definition and relabel context.                                        | **Required** - For UI its preffered to have a single REST API call for both Platform and Users defined   |
| GET     | `/alerting/rules/{alertname}/severity/{severity}/labels` | List labels (including relabel context) for the specified rule.                                   | **Required** - For UI its preffered to have a single REST API call for both Platform and Users defined   |
| GET     | `/alerting/rules/labels`                                 | List **common** labels across for selected alerts(Passes as query parameters).                    | **Required** - For UI its preffered to have a single REST API call for both Platform and Users defined   |
| GET     | `/health`                                       | Basic health‑check endpoint.                                                                      | **Required** - New                                                                                       |
| PATCH   | `/alerting/rules/{alertname}/severity/{severity}`        | Update a single rule, both platform and user-defined.                                             | **Preffered** - But for UI its preffered to have a single REST API call for both Platform and Users defined |
| POST    | `/alerting/rules`                                        | Create a new user‑defined alert rule.                                                             | **Preffered** - We will wrap the existing OC commands REST API. PATCH /namespaces/{namespace}/prometheusrules/{name} or POST /namespaces/{ns}/prometheusrules depending on final implementation decision |
| POST    | `/alerting/rules/{alertname}/severity/{severity}/clone`  | Clone an existing rule for quick customization (new name, adjusted severity, etc.).               | **Preffered** - We will wrap the existing OC commands REST API. PATCH /namespaces/{namespace}/prometheusrules/{name} or POST /namespaces/{ns}/prometheusrules depending on final implementation decision |
| PATCH   | `/alerting/rules`                                        | Bulk rule updates, like severity/status/labels, for both platform and user-defined.               | **Required** - For UI its preffered to have a single REST API call for both Platform and Users defined   |
| DELETE  | `/alerting/rules/{alertname}/severity/{severity}`        | Delete a **user‑defined** rule identified by its `alertname` and post‑relabel `severity`.         | **Required** - There is no OC command that deletes specific rules, only prometheusrules.                 |
| DELETE  | `/alerting/rules`                                        | Bulk delete **user‑defined** rules.                                                               | **Required** - There is no OC command that deletes specific rules, only prometheusrules.                 |
