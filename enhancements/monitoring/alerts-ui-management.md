...
| GET | `/rules/{ruleId}` | Fetch a single rule's full definition and relabel context. | Not yet implemented | Internal `GetRuleById` exists but is not exposed as an HTTP endpoint. |
| PATCH | `/rules/{ruleId}` | Update a single rule (platform or user-defined). | Implemented | Supports `alertingRule` (spec update), `AlertingRuleEnabled` (drop/restore), and `classification` (ARC-based label overrides). Platform operator-managed rules are updated using `AlertRelabelConfig`. |
| DELETE | `/rules/{ruleId}` | Delete a rule. | Implemented | Platform operator-managed rules: deletes via ARC drop. User-defined rules: removes from `PrometheusRule`. |
| PATCH | `/rules` | Bulk update rules (labels, classification, drop/restore). | Implemented | Per-rule status codes in response: `{ "rules": [{ "id": "...", "status_code": 204 }] }`. |
| DELETE | `/rules` | Bulk delete user-defined rules. | Implemented | Request: `{ "ruleIds": ["rid_...", ...] }`. Response: per-rule status codes. |
...
Response format (single rule update):

```json
{ "id": "rid_...", "status_code": 204 }
```
...
- Comma-separated OR within a key (e.g., `?severity=critical,warning`)
- `label.<key>=<value>` prefix syntax for label filters
