...
2. Standardize `group` and `component` labels on alerting rules to clearly surface priority and impact, to help administrators to understand what to address first.
...
#### Management Tab
This tab will include sub-tabs for **Alerting rules**, **Silence rules**, and **Manage groups**.

**Sub-tab: Alerting Rules**

- The table will list **Alerting rule name**, **Severity**, **State**, **Group**, **Component**, and **Source**.
...
- The "Append to" selection will determine which Kubernetes PrometheusRule Custom Resource will contain the alert definition. It should also include a default group and allow the user to create/update the group.
...
**Column Management**
- By default, the table will show **Alerting rule name**, **Severity**, **State**, **Group**, **Component**, and **Source**.
...
- **Alerting rule**: An alert definition. It includes a PromQL expression and an optional `for` duration and labels/annotations that emits alerts when true. Defined within a `PrometheusRule` resources.
...
- **Platform stack**: The cluster monitoring stack in the `openshift-monitoring` namespace, managed by Red Hat–supported operators.
- **User stack (User Workload Monitoring, UWM)**: The user monitoring stack for application namespaces; resources live in user namespaces and are not managed by Red Hat‑supported operators.
...
- **source label**: The origin of the alerting rule/alert (platform vs user).
- **group label**: The scope of the alert impact. It can be of `Cluster` or `Namespace` values.
- **component label**: The subsystem within the selected `group` that this alerting rule targets (e.g., api-server, etcd, network, workload).

### Alerting rule types in scope

- **Platform alerting rules**: Created and managed by Red Hat–supported operators in the platform stack.
- **Operator managed alerting rules**: Created by third‑party or otherwise unsupported operators, in user namespaces (UWM).
- **Manually created alerting rules**: Authored directly by users via `PrometheusRule` manifests in their namespaces (UWM).
...
| POST   | `/rules`                           | Create a new user‑defined alerting rule. | Wraps K8s API for `PrometheusRule`. |
...
### Data Model
- Store user alerting rules, not managed by an operator, in `PrometheusRule`s in the specified project.
- Platform alerting rules cannot be update directly. Modify platform alerting rules behavior via `AlertRelabelConfig` CRs in the `openshift-monitoring` namespace.
- Platform alerting rules can be Enabled/Disabled in the UI - which translate to a `drop` action in the `AlertRelabelConfig` CR for the alerting rule and an additional label.
