...
#### User Interface
The user interface will be redesigned, with a new **Observe > Alerting** page that highlights new grouping and components functionality. The transition strategy (feature flag, phased rollout, or direct replacement) will be determined with the Observability UI team.
...
**Actions**
- Actions will be available to **View alerting rule** and **Silence alert**.
...
- Users can **Disable/Enable** platform alerting rules using a toggle switch or kebab menu. This applies only to platform operator-managed alerting rules (via `AlertRelabelConfig`).
...
| Enabled/disabled (effective) | `AlertRelabelConfig` CR | Applies only to platform operator-managed alerting rules via an `AlertRelabelConfig` CR (drop to disable, remove the drop entry to restore). For all other rule types (user-managed, admin-created) the disable/enable toggle is not available in the UI (greyed out). |
| `openshift_io_alert_rule_id` | `PrometheusRule` CR label for user-authored rules, or API-only enrichment | Backend-computed stable identifier (`rid_<base64url-sha256>`) for correlating rules and alerts and naming platform `AlertRelabelConfig` entries. For user-authored rules created or updated by the Alerting API, the API writes the computed id into the stored rule. |
...
| Source label (`openshift_io_alert_source`) | API-enriched label | Added/derived by the Alerting API to indicate origin (e.g., platform vs user). Not overrideable. User-provided values are ignored/overwritten. Note: this label is not present in native Prometheus/Thanos alert responses; it is available only via the Alertmanager `openshift_io_alert_source` label and the Alerting API enrichment. |
...
| Effective classification (component/layer returned by API) | API-only enrichment | Computed by backend classification logic from rule labels, matcher heuristics, and user overrides stored as `AlertRelabelConfig` CRs. See [Alert Rule Classification Mapping](alert-rule-classification-mapping.md). |
...
+-----------------------+                    +---------------------------+
                  ...
                  +-------------------------+  +-----------------------+  +----------------------+
                   ...
                   CRUD |     CRU  |                       |                          |
                        v          v                       v                          v
          PrometheusRule  AlertRelabelConfig  AlertingRule   Alerts/Rules       Silences/Notifications
...
- Alerting API aggregates rule definitions and alert instances using an in-memory relabeled rules cache (built from PrometheusRules + reconciled AlertRelabelConfig Secret) to present a post-relabel view (labels, disabled status, effective severity, classification) in list/detail endpoints.
- Alerting API reads/writes `PrometheusRule` and `AlertRelabelConfig` using the Kubernetes API.
- Alerting API queries alerts/rules using Thanos/Prometheus. For cluster-scoped queries (admin), the Alerting API uses the unrestricted Thanos endpoint (port 9091). For namespace-scoped queries (developer), it uses the tenant-aware endpoint (port 9093). The endpoint is selected based on the caller's RBAC permissions.
- All requests fanned out from the Alerting API to Thanos/Prometheus and Kubernetes APIs forward the caller's bearer token to enforce proper RBAC. No credentials are stored or cached by the Alerting API.

Rationale for server‑side aggregation:
- Centralizes a canonical, post‑relabel “effective view” and transactional write path: the relabel computation (merging `PrometheusRule` definitions with `AlertRelabelConfig` state) is performed server‑side once and cached, while query results are still filtered per‑user based on forwarded credentials. This mediates validated, conflict‑aware updates to `PrometheusRule` and `AlertRelabelConfig`, preventing drift and duplicated logic across UIs.
- Being implemented in Go, leverages existing expertise and reusable Prometheus/Kubernetes client code, accelerating delivery and maintenance.
- Consolidates complex merge logic (relabel evaluation, classification heuristics, rule identity hashing) in a single component, avoiding divergent implementations across UI frontends.
...
- The Alerting API connects to whatever Thanos/Prometheus endpoints are configured. No special handling is required for HCP — the same API behavior, RBAC, and namespace scoping apply regardless of where monitoring components run.
...
- The Alerting API backend will function when the monitoring stack is installed, but console plugins and UI extensions are not officially supported on OKE. The feature may work in practice but no formal support will be provided.
...
- Acknowledge alert (requires a new persistence mechanism outside Prometheus/Alertmanager, e.g. Console‑managed annotation or ConfigMap; deferred until the concept is fully designed)
...
> The `/api/v1/alerting` prefix avoids collisions with upstream Prometheus/Thanos paths (`/api/v1/rules`, `/api/v1/alerts`) that the Console backend already proxies. For native Perses integration, GET endpoints should mimic upstream Thanos/Prometheus response schemas wherever applicable, so clients expecting Prometheus/Thanos semantics can consume this API without adapters.

Alerting Rule identity

- An alerting rule is uniquely identified by a computed, opaque `ruleId` of the form `rid_<base64url(SHA256(canonical_payload))>`.
- The canonical payload is derived from the rule's identity and spec fields, concatenated with `\n---\n` separators:
  ...
  - `labels`: business labels only (excluding `openshift_io_*` and `alertname`), validated for Prometheus syntax, empty values dropped, sorted by key ascending, joined as `key=value` lines separated by `\n`
- Annotations are excluded from the hash, so annotation-only changes do not alter the rule ID.
- Clients must treat `ruleId` as opaque (do not construct it). When `name`/`expr`/`for`/labels change, the server recomputes the ID and returns an updated `ruleId`.

** Alerting Rules (definitions)**

| Method | Path | Description | Status | Notes |
|--------|------|-------------|--------|-------|
| GET | `/rules` | List all alerting rule definitions **(platform and user)**, including relabel context. Supports filter parameters listed below. | Implemented | Merges relabeled rules (from `alert-relabel-configs` Secret) with Prometheus rules API. |
| GET | `/rules/labels` | List common labels across selected rules **(platform and user)**. | Not yet implemented | |
| POST | `/rules` | Create a new alerting rule. | Implemented | User stack: add rule to `PrometheusRule` in target namespace. Platform stack (admin): create/update `AlertingRule` CR `platform-alert-rules` in `openshift-monitoring`. Request body: `{ "alertingRule": {...}, "prometheusRule": {"prometheusRuleName": "...", "prometheusRuleNamespace": "...", "groupName": "..."} }`. Response: `{ "id": "rid_..." }`. |
| PATCH | `/rules` | Update rules (labels, classification, drop/restore). Accepts one or more rules. | Implemented | Per-rule status codes in response: `{ "rules": [{ "id": "...", "status_code": 204 }] }`. |
| DELETE | `/rules` | Delete user-defined rules. Accepts one or more rule IDs. | Implemented | Request: `{ "ruleIds": ["rid_...", ...] }`. Response: per-rule status codes. |
...
The server persists the disablement using an `AlertRelabelConfig` CR (drop action keyed by rule ID).
...
The server restores the original behavior by removing the drop action from the corresponding `AlertRelabelConfig` CR.

- Update classification labels on a platform alerting rule:
...
The server creates or updates an `AlertRelabelConfig` CR with Replace actions for the classification labels.

Response format:

```json
{ "rules": [{ "id": "rid_...", "status_code": 204 }] }
```

Filter parameters for `/rules`:

Current implementation:
- `state` (one of: `firing`, `pending`, `silenced`, or empty for all)
- Any other query key is treated as a label equality filter (e.g., `?namespace=openshift-monitoring&severity=critical`)
- Semantics: AND across different keys, exact matches only
- No server-side pagination or sorting

Implemented:
- Label matchers following upstream Prometheus API conventions via `match[]` query parameter (see [Prometheus rules API](https://prometheus.io/docs/prometheus/latest/querying/api/#rules)). Supports equality, inequality, regex, and negative-regex matchers, e.g. `match[]=severity="critical"&match[]=alertname=~"Kube.*"`

Planned (not yet implemented):
- `has_silenced_alerts` (boolean)
- Pagination: `limit=<int>`, `continue=<opaque>`
...
| Method | Path | Description | Status | Notes |
|--------|------|-------------|--------|-------|
| GET | `/alerts` | List alert instances (Pending / Firing / Silenced) with post-relabel labels. | Implemented | Prometheus-compatible response plus additive JSON fields `alertRuleId`, `alertComponent`, `alertLayer`, `prometheusRuleName`, `prometheusRuleNamespace`. See [Classification doc](alert-rule-classification-mapping.md). |
...
- Classification mapping (both stacks)
  - Action: Classification is computed server-side using matcher heuristics and an in-memory relabeled rules cache. Defaults are not persisted.
  - User override: When a user updates a rule's classification via the PATCH API, the backend creates or updates an `AlertRelabelConfig` CR in the appropriate monitoring namespace (see [Alert Rule Classification Mapping](alert-rule-classification-mapping.md) for details). The ARC contains relabel configs that set classification labels (`openshift_io_alert_rule_component`, `openshift_io_alert_rule_layer`) on the matched rule. CMO reconciles the ARC into the Prometheus relabel pipeline.

### Implementation Details/Notes/Constraints
- Server-side aggregation computes a canonical, post‑relabel view for both stacks.
- The `ruleId` (format: `rid_<base64url-sha256>`) is treated as an opaque identifier. Any change to `name`/`expr`/`for`/business labels yields a new `ruleId`.
