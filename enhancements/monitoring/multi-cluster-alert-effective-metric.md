...
- On **OCP spokes**, the metric carries post-ARC labels (`openshift_io_alert_rule_id`, component, layer), silence state (`alertstate=silenced`), and excludes disabled (ARC-dropped) alerts.
- On **non-OCP spokes**, the metric may have reduced label coverage — no ARC-applied labels unless classification is embedded directly in PrometheusRule labels. Silence state depends on the spoke's monitoring stack configuration. The metric is available only if the spoke runs the monitoring-plugin or an equivalent exporter that produces it.
- On **OCP spokes without the monitoring-plugin**, the metric is not available. Historical alert views for these spokes fall back to the raw `ALERTS` metric in hub Thanos, with the limitations documented in the parent proposal.
...
**Spokes without the monitoring-plugin:** The `alerts_effective_*` metric requires the monitoring-plugin on the spoke. Non-OCP spokes and OCP spokes that don't run it fall back to the raw `ALERTS` metric for historical views, with reduced label coverage (no ARC labels, no silence awareness). The real-time alerts page (hub AM) is unaffected — it works for all spoke types.
...
- **Incomplete historical coverage in heterogeneous fleets**: non-OCP spokes and OCP spokes without the monitoring-plugin may not produce the `alerts_effective_*` metric. Dashboards and historical queries must handle missing data gracefully (e.g., show a "limited data" indicator for spokes without enriched history).
...
- **Non-OCP spoke metric availability:** Non-OCP spokes don't run the monitoring-plugin and may not produce the `alerts_effective_*` metric. Should the hub generate a synthetic version from hub AM data for these spokes (covering the gap at the cost of ~5 min latency), or accept that historical enriched views require the monitoring-plugin? If synthetic, this could also solve the gap for OCP spokes without the monitoring-plugin.

- **Alert forwarding for non-OCP spokes:** OCP uses `additionalAlertmanagerConfigs` injected by the MCOA endpoint operator. Non-OCP spokes need an alternative path to forward alerts to hub AM. Should MCOA handle this automatically, or is it a manual spoke configuration step?
