...
The multi-cluster alerting experience supports heterogeneous fleets:
- **OCP with spoke Alerting API** — full feature set including spoke Alerting API optimizations, ARC-based classification, and ClusterOperator topology discovery.
- **Older OCP versions** — hub manages spoke rules via direct kube API through ManagedClusterProxy. No spoke Alerting API required. ARC-based classification available.
- **Non-OCP Kubernetes** (EKS, AKS, GKE, vanilla k8s) — hub manages spoke rules via direct kube API. Classification uses PrometheusRule annotations or hub-side mapping (no AlertRelabelConfig CRD). Requires prometheus-operator on the spoke for PrometheusRule CRD support. Monitoring namespace is configurable per managed cluster.
...
7. Support heterogeneous fleets: OCP (any version) and non-OCP Kubernetes clusters, with graceful degradation based on each spoke's available capabilities.
...
- Supporting monitoring stacks without the PrometheusRule CRD (e.g., raw Prometheus with file-based rules). prometheus-operator is the minimum spoke requirement.
...
The hub monitoring-plugin reuses the **same single-cluster Alerting API** (`/api/v1/alerting/*`). On the hub, the monitoring-plugin is configured to point to hub Alertmanager as its backend, which receives both spoke-forwarded alerts and ThanosRuler alerts. The enrichment pipeline adds spoke rule metadata and classification to produce a unified fleet view through the same API surface. Spoke clusters forward alerts and metrics to the hub; the hub classifies, enriches, and serves a unified view. See the summaries below and child proposals for details on each flow.

#### Hub-Centric Spoke Management via Direct Kube API

The hub manages spoke alert rules primarily via **direct Kubernetes API** access through ManagedClusterProxy — creating, updating, and deleting `PrometheusRule` CRDs on spoke clusters. This is the universal path that works on any managed cluster with the PrometheusRule CRD (prometheus-operator), regardless of OCP version or platform.

When the spoke Alerting API is available, the hub can optionally use it for enriched operations: classification cache retrieval, server-side validation, and health status. This is an optimization, not a requirement.

#### Cluster Capability Detection

The hub probes each managed cluster at registration and periodically to detect available capabilities:

| Capability | Detection | Impact |
|------------|-----------|--------|
| PrometheusRule CRD | API discovery | Can manage alert rules |
| AlertRelabelConfig CRD | API discovery | Can use ARC-based classification persistence |
| ClusterOperator resources | API discovery | Can do operator topology discovery for classification |
| monitoring-plugin Alerting API | Health endpoint probe | Can use enriched operations (classification cache, validated writes) |
| Monitoring namespace | ManagedCluster label or auto-detection | Where to create/read resources |

Results are cached per managed cluster and refreshed periodically. The hub adapts its behavior per spoke based on detected capabilities.
...
**Prerequisites**: ManagedClusterProxy, MCOA endpoint operator. Spoke Alerting API is optional — enables enriched operations (classification cache, validated writes) but is not required. Non-OCP clusters require prometheus-operator for PrometheusRule CRD support.
...
#### Non-OCP Kubernetes Clusters (EKS, AKS, GKE, vanilla k8s)

Non-OCP clusters registered with ACM are supported with the following constraints:
- **Rule management**: Hub manages PrometheusRule CRDs via direct kube API through ManagedClusterProxy. Requires prometheus-operator on the spoke.
- **Classification**: No AlertRelabelConfig CRD available. Classification uses PrometheusRule annotations or hub-side ConfigMap mapping.
- **Monitoring namespace**: Not `openshift-monitoring`. Configurable per managed cluster via ManagedCluster label or hub-side configuration.
- **Alert forwarding**: Spoke Alertmanager must be configured to forward alerts to hub AM (via MCOA addon or manual configuration).
- **Metrics federation**: MCOA metrics-collector federates spoke metrics to hub Thanos. `alerts_effective_*` metric availability depends on the spoke monitoring stack.

#### OCP Clusters Without Spoke Alerting API

Fully supported via direct kube API. ARC-based classification is available (AlertRelabelConfig CRD exists). Classification cache and validated writes are handled hub-side.
...
- **Heterogeneous fleet complexity**: different cluster types (OCP versions, non-OCP) have different capabilities. The hub must adapt per spoke. Mitigated by cluster capability detection and a clear fallback hierarchy (spoke API → direct kube API → degraded mode).
- **Non-OCP classification gap**: without ARC CRDs, classification on non-OCP clusters is limited to rule-embedded labels or hub-side mapping. Users must be informed of reduced classification coverage on non-OCP spokes.
