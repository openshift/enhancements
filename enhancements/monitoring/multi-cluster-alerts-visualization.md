...
- **Spoke Alerting API (monitoring-plugin)**: The Rule Metadata Cache can use each spoke's Alerting API (`GET /alerting/rules`) for rule metadata (`alertRuleId`, `source`, `managedBy`, `prometheusRuleName`, `disabled`). When the spoke Alerting API is not available, rule metadata is retrieved directly from `PrometheusRule` CRDs via the kube API through ManagedClusterProxy. Spokes without the monitoring-plugin will still get classification (from the hub-side classifier) but may have reduced rule metadata.
